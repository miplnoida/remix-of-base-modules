      ...
      <DialogContent className="max-w-[95vw] w-[1200px] h-[90vh] flex flex-col p-0 gap-0">
        ...
        {!hasConfiguredDepts ? (
          ...
          <div className="flex-1 min-h-0 flex flex-col lg:flex-row">
                ...
                <div className="space-y-1.5 flex-1 min-h-0">
                  ...
                  {usersInDept.length > 0 ? (
                    <RadioGroup value={selectedUserId} onValueChange={(v) => { setSelectedUserId(v); setSelectedTime(''); setMeetingDate(undefined); setOverlapError(''); }} className="space-y-1.5">
                      {usersInDept.map((u) => (
                        <Label key={u.id} htmlFor={`user-${u.id}`} className={cn('flex items-center gap-2.5 p-2 border rounded-lg cursor-pointer transition-colors text-sm', selectedUserId === u.id ? 'border-primary bg-primary/5' : 'border-border hover:border-primary/50')}>
            ...
            {/* RIGHT PANEL: Calendar + Time + Remarks */}
                ...
                <>
                      ...
                      {Array.from({ length: 28 }, (_, i) => {
                        ...
                        elements.push(
                          <button key={dayStr} type="button"
                            onClick={() => { setMeetingDate(day); setSelectedTime(''); setOverlapError(''); }}
                  ...
                  {meetingDate && (
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 flex-1 min-h-0">
                      {/* Existing meetings */}
                      <div className="space-y-2">
                        <Label className="text-xs font-semibold">Meetings on {formatDisplayDate(meetingDate)}</Label>
                        ...
                        <div className="grid grid-cols-3 sm:grid-cols-4 gap-1">
                          {timeSlots.map((slot) => {
                            const isOccupied = userMeetings.some((m: any) => {
                              ...
                              // True overlap: [slotMin, slotEndMin) intersects [meetingMin, meetingEndMin)
