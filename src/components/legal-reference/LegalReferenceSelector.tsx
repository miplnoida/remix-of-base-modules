/**
 * LegalReferenceSelector — module-agnostic searchable selector over
 * `legal_reference`. Filters by country and (optionally) tags.
 */
...
export interface LegalReferenceSelectorProps {
  ...
  /** Restrict to references having ANY of these tags. */
  tags?: string[];
  /** Include DRAFT/SUPERSEDED/REPEALED in the list. */
...
export const LegalReferenceSelector: React.FC<LegalReferenceSelectorProps> = ({
  value, onChange, countryCode, tags, includeNonActive = false,
  placeholder = 'Select legal reference…', disabled, required, className,
}) => {
  const [open, setOpen] = useState(false);

  const { data: refs = [] } = useQuery({
    queryKey: ['legal-ref-selector', countryCode, includeNonActive, tags?.join(',') || ''],
    enabled: !!countryCode,
    queryFn: async () => {
      let q = db
      ...
      if (tags?.length) {
        ...
        rows = rows.filter((r) => (r.tags ?? []).some((t: string) => wanted.has(t)));
      }
      return rows;
    },
  });
          ...
          <span className="flex items-center gap-2 min-w-0 truncate">
            ...
            {selected ? (
              ...
              <span className="text-muted-foreground">
                {!countryCode ? 'Pick a country first' : placeholder}
              </span>
            )}
          </span>
