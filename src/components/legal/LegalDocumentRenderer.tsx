...
function tokenValue(path: string, ctx: any, t: LegalRenderTokens): string {
  ...
  const map: Record<string, any> = {
    organization: {
      ...
      logo: ctx?.organization?.logoUrl,
      seal: ctx?.organization?.sealUrl,
    },
    department: {
      name: ctx?.department?.name,
      code: ctx?.department?.code,
      manager: ctx?.department?.headName ?? ctx?.department?.managerName,
    },
    location: {
    ...
    letterhead: {
      header: ctx?.letterhead?.headerHtml ?? "",
      footer: ctx?.letterhead?.footerHtml ?? "",
    },
    email: { signature: ctx?.email_signature?.signatureHtml ?? ctx?.email_signature?.signatureText ?? "" },
    disclaimer: { standard: ctx?.disclaimer?.html ?? ctx?.disclaimer?.text ?? "" },
    case: t.case ?? {},
    party: t.party ?? {},
    hearing: t.hearing ?? {},
    order: t.order ?? {},
