...
import { useIAFindings, useIAActions } from '@/hooks/useAuditDataExtended';
...
export default function CommitteeReports() {
  ...
  const handleExport = (reportName: string) => {
    toast({ title: 'Export', description: `${reportName} export will be available in PDF format.` });
  };

  const reports = [
    { title: 'Committee Summary Report', description: 'Overview of audit activities, key findings, and management actions for the reporting period.', count: engagements.length, icon: FileText },
    { title: 'Open Critical Issues', description: 'All open findings rated Critical or High that require immediate attention.', count: criticalFindings.length, icon: AlertTriangle },
    { title: 'High-Risk Entity List', description: 'Entities rated as High or Critical risk from the audit universe.', count: highRiskEntities.length, icon: AlertTriangle },
    { title: 'Overdue Management Responses', description: 'Management actions and responses that have exceeded their target dates.', count: overdueActions.length, icon: Clock },
    { title: 'Audit Plan Progress', description: 'Status of all planned engagements against the annual audit plan.', count: engagements.filter((e: any) => e.status === 'Closed').length, icon: FileText },
  ];
