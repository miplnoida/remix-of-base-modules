...
export default function WorkloadCapacity() {
  ...
  if (loadingAuditors || loadingActivities) return <div className="p-6">Loading...</div>;

  const auditorWorkloads = auditors.map((auditor: any) => {
    const assignedActivities = activities.filter(
      (act: any) => act.auditor_id === auditor.id
    );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-medium">Total Auditors</CardTitle><Users className="h-4 w-4 text-muted-foreground" /></CardHeader>
          <CardContent><div className="text-2xl font-bold">{auditors.length}</div><p className="text-xs text-muted-foreground">Active auditors</p></CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-medium">Total Activities</CardTitle><TrendingUp className="h-4 w-4 text-muted-foreground" /></CardHeader>
          <CardContent><div className="text-2xl font-bold">{activities.length}</div><p className="text-xs text-muted-foreground">Assigned activities</p></CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-medium">Avg. Utilization</CardTitle><Clock className="h-4 w-4 text-muted-foreground" /></CardHeader>
          <CardContent><div className="text-2xl font-bold">{auditorWorkloads.length > 0 ? Math.round(auditorWorkloads.reduce((s: number, a: any) => s + a.utilizationRate, 0) / auditorWorkloads.length) : 0}%</div><p className="text-xs text-muted-foreground">Capacity used</p></CardContent>
        </Card>
        <Card>
          ...
          <CardContent><div className="text-2xl font-bold">{auditorWorkloads.reduce((s: number, a: any) => s + a.completedActivities, 0)}</div><p className="text-xs text-muted-foreground">Activities done</p></CardContent>
        </Card>
      </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-4">
                    <div className="text-right">
                      <div className="text-sm font-medium">{auditor.assignedHours} / {auditor.totalHours} hours</div>
                      <div className="text-xs text-muted-foreground">{auditor.availableHours} hours available</div>
                    </div>
                    <Badge className={getUtilizationColor(auditor.utilizationRate)}>{Math.round(auditor.utilizationRate)}% Utilized</Badge>
                  </div>
                </div>
