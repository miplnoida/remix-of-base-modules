...
const emptyForm = {
  test_date: new Date().toISOString().slice(0, 10), tested_by: '', sample_size: 0,
  exceptions_found: 0, result: 'Not Tested', remarks: '',
};
...
export default function ControlTesting() {
  ...
  const filtered = data.filter((r: any) => {
    ...
    const ms = !s || r.tested_by?.toLowerCase().includes(s) || r.remarks?.toLowerCase().includes(s);
    const mR = filters.result === 'all' || r.result === filters.result;
    return ms && mR;
  });

  const stats = {
    total: data.length,
    pass: data.filter((d: any) => d.result === 'Pass').length,
    fail: data.filter((d: any) => d.result === 'Fail').length,
    pending: data.filter((d: any) => d.result === 'Not Tested').length,
  };

  const openAdd = () => { setForm(emptyForm); setModalState({ mode: 'add' }); };
  const openEdit = (r: any) => {
    setForm({ test_date: r.test_date || '', tested_by: r.tested_by || '', sample_size: r.sample_size || 0, exceptions_found: r.exceptions_found || 0, result: r.result || 'Not Tested', remarks: r.remarks || '' });
  ...
  const filterFields: StandardFilterField[] = [
    { key: 'result', label: 'Result', options: [{ label: 'All', value: 'all' }, ...RESULTS.map(r => ({ label: r, value: r }))] },
  ];
  ...
  return (
    <PageShell title="Control Testing" subtitle="Test and evaluate control effectiveness"
      ...
      <Card><CardContent className="p-4">
        <StandardSearchFilterBar searchTerm={searchTerm} onSearchChange={setSearchTerm} searchPlaceholder="Search tests..." filters={filters} onFilterChange={(k, v) => setFilters(f => ({ ...f, [k]: v }))} filterFields={filterFields} onReset={() => { setSearchTerm(''); setFilters({ result: 'all' }); }} />
      </CardContent></Card>
      ...
      <StandardModal open={modalState.mode !== null} onOpenChange={() => setModalState({ mode: null })}
        title={modalState.mode === 'add' ? 'New Control Test' : modalState.mode === 'edit' ? 'Edit Test' : 'View Test'}
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div><Label>Exceptions Found</Label><Input type="number" value={form.exceptions_found} onChange={e => setForm(f => ({ ...f, exceptions_found: Number(e.target.value) }))} disabled={isReadOnly} /></div>
            <div><Label>Result</Label>
              <Select value={form.result} onValueChange={v => setForm(f => ({ ...f, result: v }))} disabled={isReadOnly}>
