...
export default function RiskAssessment() {
  ...
  const filtered = data.filter((r: any) => {
    ...
    const matchSearch = !s || r.assessed_by?.toLowerCase().includes(s) || r.notes?.toLowerCase().includes(s);
  ...
  return (
    <PageShell title="Risk Assessment" subtitle="Assess, score, and prioritize auditable entities"
          ...
          <div className="grid grid-cols-2 gap-4">
            <div><Label>Auditable Entity</Label>
              <Select value={form.audit_universe_id} onValueChange={v => setFormField('audit_universe_id', v)} disabled={isReadOnly}>
                ...
                <SelectContent>{universeData.map((e: any) => <SelectItem key={e.id} value={e.id}>{e.entity_name}</SelectItem>)}</SelectContent>
              </Select>
            </div>
