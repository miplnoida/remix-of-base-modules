...
const emptyForm = { program_name: '', program_code: '', audit_area: '', objective: '', scope: '', methodology: '', status: 'Draft', version: 1 };
...
export default function AuditPrograms() {
  ...
  const filtered = data.filter((r: any) => {
    ...
    return (!s || r.program_name?.toLowerCase().includes(s) || r.program_code?.toLowerCase().includes(s)) && (filters.status === 'all' || r.status === filters.status);
  });
  ...
  const handleDuplicate = (r: any) => { create.mutate({ program_name: `${r.program_name} (Copy)`, program_code: '', audit_area: r.audit_area, objective: r.objective, scope: r.scope, methodology: r.methodology, status: 'Draft', version: (r.version || 1) + 1, ...getCreateFields() } as any); };
  ...
  const columns: DataTableColumn<any>[] = [
    { key: 'program_code', header: 'Code' }, { key: 'program_name', header: 'Program Name' },
    { key: 'audit_area', header: 'Audit Area' }, { key: 'version', header: 'Version' },
  ...
  return (
    <PageShell title="Audit Programs" subtitle="Maintain reusable audit programs and procedure templates"
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <MetricCard title="Total Programs" value={data.length} icon={BookOpen} variant="info" />
        <MetricCard title="Active" value={data.filter((d: any) => d.status === 'Active').length} icon={FileCheck} variant="success" />
        <MetricCard title="Draft" value={data.filter((d: any) => d.status === 'Draft').length} icon={Copy} variant="default" />
      </div>
      <Card><CardContent className="p-4">
        <StandardSearchFilterBar searchValue={searchTerm} onSearchChange={setSearchTerm} searchPlaceholder="Search programs..." filterValues={filters} onFilterChange={(k, v) => setFilters(f => ({ ...f, [k]: v }))} filters={filterFields} onReset={() => { setSearchTerm(''); setFilters({ status: 'all' }); }} />
      </CardContent></Card>
      <Card><CardContent>
        <DataTable columns={columns} data={filtered} onView={openView}
          renderActions={(row) => (<div className="flex gap-1"><Button variant="ghost" size="sm" onClick={(e) => { e.stopPropagation(); openEdit(row); }}>Edit</Button><Button variant="ghost" size="sm" onClick={(e) => { e.stopPropagation(); handleDuplicate(row); }}><Copy className="h-3 w-3 mr-1" />Duplicate</Button></div>)} />
      </CardContent></Card>
      <StandardModal open={modalState.mode !== null} onOpenChange={() => setModalState({ mode: null })}
        title={modalState.mode === 'create' ? 'New Audit Program' : modalState.mode === 'edit' ? 'Edit Program' : 'View Program'}
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div><Label>Status</Label><Select value={form.status} onValueChange={v => setForm(f => ({ ...f, status: v }))} disabled={isReadOnly}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{STATUSES.map(s => <SelectItem key={s} value={s}>{s}</SelectItem>)}</SelectContent></Select></div>
          </div>
