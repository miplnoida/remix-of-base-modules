...
const emptyForm = { work_date: new Date().toISOString().slice(0, 10), hours_spent: 0, work_type: 'Fieldwork', notes: '', engagement_id: '' };
...
export default function TimeTracking() {
  ...
  const filtered = data.filter((r: any) => (!searchTerm || r.notes?.toLowerCase().includes(searchTerm.toLowerCase())) && (filters.work_type === 'all' || r.work_type === filters.work_type));
  ...
  const openEdit = (r: any) => { setForm({ work_date: r.work_date || '', hours_spent: r.hours_spent || 0, work_type: r.work_type || 'Fieldwork', notes: r.notes || '', engagement_id: r.engagement_id || '' }); setModalState({ mode: 'edit', record: r }); };
  const handleSave = () => {
  ...
  const columns: DataTableColumn<any>[] = [{ key: 'work_date', header: 'Date' }, { key: 'hours_spent', header: 'Hours' }, { key: 'work_type', header: 'Type' }, { key: 'notes', header: 'Notes' }];
      ...
      <StandardModal open={modalState.mode !== null} onOpenChange={() => setModalState({ mode: null })}
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div><Label>Engagement</Label><Select value={form.engagement_id} onValueChange={v => setForm(f => ({ ...f, engagement_id: v }))}><SelectTrigger><SelectValue placeholder="Optional" /></SelectTrigger><SelectContent>{engagements.map((e: any) => <SelectItem key={e.id} value={e.id}>{e.engagement_name}</SelectItem>)}</SelectContent></Select></div>
          </div>
