...
import { BarChart3, AlertTriangle, CheckCircle, Clock, TrendingUp, Users, Shield, FileText } from 'lucide-react';
        ...
        <Card>
          ...
          <CardContent>
            {findingsBySeverity.length > 0 ? (
              <ResponsiveContainer width="100%" height={300}>
                <PieChart>
                  <Pie data={findingsBySeverity} cx="50%" cy="50%" outerRadius={100} dataKey="value" nameKey="name" label>
                    {findingsBySeverity.map((_, index) => <Cell key={index} fill={COLORS[index % COLORS.length]} />)}
                  </Pie>
                  <Tooltip />
                  <Legend />
                </PieChart>
