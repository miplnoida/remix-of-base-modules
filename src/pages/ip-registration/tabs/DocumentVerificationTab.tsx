...
export default function DocumentVerificationTab({ formData, onChange, onSave, errors, isEditable, clearError }: DocumentVerificationTabProps) {
  ...
  const fetchDocuments = useCallback(async () => {
    try {
      // Query ip_application_documents by SSN or by unique_uuid in metadata
      let query = supabase
        .from('ip_application_documents')
      ...
      } else if (formData.unique_uuid) {
        query = query.contains('metadata', { unique_uuid: formData.unique_uuid });
      } else {
        setDocuments([]);
        setLoading(false);
        return;
    ...
    for (const file of Array.from(files)) {
      ...
      try {
        ...
        const { error: dbError } = await supabase
          ...
          .insert({
            ssn: ssn || null,
            application_reference_number: (formData as any).application_reference_number || null,
