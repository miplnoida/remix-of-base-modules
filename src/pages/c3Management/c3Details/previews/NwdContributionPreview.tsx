...
const NwdContributionPreview: React.FC<Props> = ({ open, onClose, data, loading, companyName, registrationNumber }) => {
  ...
  const handlePrint = () => {
    ...
    if (!win) return;
    win.document.write(`<html><head><title>NW Director Report</title><style>
                ...
                <tbody>
                  {directors.map((d: any, idx: number) => (
                    <tr key={idx}>
                      ...
                      <td className="border border-black p-1 text-right">{fmt(d.week1)}</td>
                      <td className="border border-black p-1 text-right">{fmt(d.week2)}</td>
...
export default NwdContributionPreview;
