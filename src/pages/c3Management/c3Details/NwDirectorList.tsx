...
import { Search, Eye, Trash2, CheckCircle2, XCircle, Printer, Home, RefreshCw } from 'lucide-react';
import { toast } from 'sonner';
import { format, parseISO } from 'date-fns';
import { Breadcrumb, BreadcrumbItem, BreadcrumbList, BreadcrumbPage, BreadcrumbSeparator } from '@/components/ui/breadcrumb';
import { useNavigate } from 'react-router-dom';
import {
  getNwdContributionList,
  deleteContribution,
...
const NwDirectorList: React.FC = () => {
  ...
  const [contributions, setContributions] = useState<NwdContributionRecord[]>([]);
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await deleteContribution(deleteId, 'nwd');
  ...
  const handlePay = (record: NwdContributionRecord) => {
    ...
    <div className="p-6 space-y-4">
      <Breadcrumb>
        <BreadcrumbList>
          ...
          <BreadcrumbSeparator />
          <BreadcrumbItem><BreadcrumbPage>Non Working Director</BreadcrumbPage></BreadcrumbItem>
        </BreadcrumbList>
      </Breadcrumb>
                ...
                <TableBody>
                  {contributions.map((c) => (
                    <TableRow key={`${c.header_id}-${c.schedule}`}>
                      ...
                      <TableCell>{c.is_nil ? <CheckCircle2 className="h-4 w-4 text-green-500" /> : <XCircle className="h-4 w-4 text-red-500" />}</TableCell>
