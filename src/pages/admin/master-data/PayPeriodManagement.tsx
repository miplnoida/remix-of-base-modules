...
const TABLE = "tb_pay_periods";
const MODULE_NAME = "md_pay_periods";
const TITLE = "Pay Periods";
...
const PayPeriodManagement = () => {
  ...
  const [form, setForm] = useState({ code: "", description: "", sort_order: "0", is_active: true });
  ...
  const deleteMutation = useMutation({
    mutationFn: async (code: string) => {
      const { error } = await (supabase as any).from(TABLE).delete().eq("code", code);
      if (error) throw error;
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: [TABLE] });
  ...
  const openAdd = () => {
    setEditing(null);
    setForm({ code: "", description: "", sort_order: "0", is_active: true });
    setShowDialog(true);
  };

  const openEdit = (r: Row) => {
    setEditing(r);
    setForm({ code: r.code, description: r.description || "", sort_order: String(r.sort_order || 0), is_active: r.is_active ?? true });
    setShowDialog(true);
  };
      ...
      <div className="container mx-auto p-6 space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground mt-1">Manage pay period master data</p>
          </div>
          {can("create") && (
                    ...
                    <TableRow key={r.code}>
                      ...
                      <TableCell className="text-right">
                        <div className="flex justify-end gap-1">
                          <Button variant="ghost" size="icon" onClick={() => { setViewing(r); setShowView(true); }}>
                          ...
                          {can("delete") && (
                            <Button variant="ghost" size="icon" onClick={() => { setDeleteId(r.code); setShowDelete(true); }}>
          ...
          <DialogContent className="max-w-md">
            ...
            {viewing && (
              <div className="space-y-3 py-2">
                ...
                <div><Label className="text-muted-foreground text-xs">Sort Order</Label><p className="font-medium">{viewing.sort_order}</p></div>
                <div><Label className="text-muted-foreground text-xs">Active</Label><p className="font-medium">{viewing.is_active ? "Yes" : "No"}</p></div>
                <div><Label className="text-muted-foreground text-xs">Entered By</Label><p className="font-medium">{viewing.entered_by || "-"}</p></div>
        ...
        <Dialog open={showDialog} onOpenChange={setShowDialog}>
          <DialogContent className="max-w-md">
            <DialogHeader>
              ...
              <DialogDescription>{editing ? "Update details" : "Create a new record"}</DialogDescription>
            </DialogHeader>
            <div className="grid gap-4 py-4">
              <div className="space-y-2">
                <Label>Code *</Label>
                <Input value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value })} disabled={!!editing} maxLength={5} />
              </div>
              <div className="space-y-2">
