...
interface Row { code: string; description: string; isactive: boolean; }
const TABLE = "tb_c3_status"; const MODULE_NAME = "md_c3_status"; const TITLE = "C3 Status";

const C3StatusManagement = () => {
  ...
  const [form, setForm] = useState({ code: "", description: "", isactive: true });
  ...
  const openAdd = () => { setEditing(null); setForm({ code: "", description: "", isactive: true }); setShowDialog(true); };
  const openEdit = (r: Row) => { setEditing(r); setForm({ code: r.code, description: r.description, isactive: r.isactive }); setShowDialog(true); };

  return (
    <PermissionWrapper moduleName={MODULE_NAME}>
      <div className="container mx-auto p-6 space-y-6">
        <div className="flex items-center justify-between"><div><h1 className="text-3xl font-bold text-foreground">{TITLE}</h1><p className="text-muted-foreground mt-1">Manage C3 status master data</p></div>{can("create") && <Button onClick={openAdd}><Plus className="mr-2 h-4 w-4" /> Add Record</Button>}</div>
...
export default C3StatusManagement;
