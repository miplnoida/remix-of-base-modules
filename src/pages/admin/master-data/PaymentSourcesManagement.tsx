...
const MODULE_NAME = "md_payment_sources";

const PaymentSourcesManagement = () => {
  ...
  const { data: rows = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await (supabase as any).from("tb_payment_sources").select("*").order("code");
  ...
  const saveMutation = useMutation({
    mutationFn: async (r: typeof form) => {
      if (editing) {
        const { error } = await (supabase as any).from("tb_payment_sources").update({ description: r.description, is_active: r.is_active, updated_by: userCode, updated_at: new Date().toISOString() }).eq("code", r.code);
        if (error) throw error;
      } else {
        const { error } = await (supabase as any).from("tb_payment_sources").insert({ code: r.code, description: r.description, is_active: r.is_active, created_by: userCode, created_at: new Date().toISOString() });
        if (error) throw error;
      }
    },
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["tb_payment_sources"] }); toast.success(editing ? "Updated" : "Created"); setShowDialog(false); setEditing(null); },
    onError: (e: any) => toast.error("Save failed: " + (e.message || "Unknown error")),
  });

  const deleteMutation = useMutation({
    mutationFn: async (code: string) => { const { error } = await (supabase as any).from("tb_payment_sources").delete().eq("code", code); if (error) throw error; },
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["tb_payment_sources"] }); toast.success("Deleted"); setShowDelete(false); setDeleteId(null); },
    ...
    <PermissionWrapper moduleName={MODULE_NAME}>
      <div className="container mx-auto p-6 space-y-6">
        <div className="flex items-center justify-between">
          <div><h1 className="text-3xl font-bold text-foreground">Payment Sources</h1><p className="text-muted-foreground mt-1">Manage payment source master data</p></div>
          {can("create") && <Button onClick={openAdd}><Plus className="mr-2 h-4 w-4" /> Add Payment Source</Button>}
        </div>
        <Card>
          <CardHeader><CardTitle>Payment Sources</CardTitle><CardDescription>All payment sources in the system</CardDescription>
        ...
        <Dialog open={showView} onOpenChange={setShowView}><DialogContent className="max-w-md"><DialogHeader><DialogTitle>View Payment Source</DialogTitle></DialogHeader>
        ...
        <Dialog open={showDialog} onOpenChange={setShowDialog}><DialogContent className="max-w-md"><DialogHeader><DialogTitle>{editing ? "Edit Payment Source" : "Add Payment Source"}</DialogTitle><DialogDescription>{editing ? "Update details" : "Create a new record"}</DialogDescription></DialogHeader>
        ...
        <AlertDialog open={showDelete} onOpenChange={setShowDelete}><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Delete Payment Source</AlertDialogTitle><AlertDialogDescription>Are you sure? This cannot be undone.</AlertDialogDescription></AlertDialogHeader>
...
export default PaymentSourcesManagement;
