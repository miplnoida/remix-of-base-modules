...
const MODULE_NAME = "md_invoice_status";

const InvoiceStatusManagement = () => {
  ...
  const { data: rows = [], isLoading } = useQuery({
    queryKey: ["tb_invoice_status"],
    queryFn: async () => {
      const { data, error } = await (supabase as any).from("tb_invoice_status").select("*").order("code");
  ...
  const saveMutation = useMutation({
    mutationFn: async (r: typeof form) => {
      if (editing) {
        const { error } = await (supabase as any).from("tb_invoice_status").update({ description: r.description, is_active: r.is_active, updated_by: userCode, updated_at: new Date().toISOString() }).eq("code", r.code);
        if (error) throw error;
      } else {
        const { error } = await (supabase as any).from("tb_invoice_status").insert({ code: r.code, description: r.description, is_active: r.is_active, created_by: userCode, created_at: new Date().toISOString() });
        if (error) throw error;
      }
    },
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["tb_invoice_status"] }); toast.success(editing ? "Updated" : "Created"); setShowDialog(false); setEditing(null); },
    onError: (e: any) => toast.error("Save failed: " + (e.message || "Unknown error")),
  });

  const deleteMutation = useMutation({
    mutationFn: async (code: string) => { const { error } = await (supabase as any).from("tb_invoice_status").delete().eq("code", code); if (error) throw error; },
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["tb_invoice_status"] }); toast.success("Deleted"); setShowDelete(false); setDeleteId(null); },
    ...
    <PermissionWrapper moduleName={MODULE_NAME}>
      <div className="container mx-auto p-6 space-y-6">
        <div className="flex items-center justify-between">
          <div><h1 className="text-3xl font-bold text-foreground">Invoice Status</h1><p className="text-muted-foreground mt-1">Manage invoice status master data</p></div>
          {can("create") && <Button onClick={openAdd}><Plus className="mr-2 h-4 w-4" /> Add Invoice Status</Button>}
        </div>
        <Card>
          <CardHeader><CardTitle>Invoice Statuses</CardTitle><CardDescription>All invoice statuses in the system</CardDescription>
        ...
        <Dialog open={showView} onOpenChange={setShowView}><DialogContent className="max-w-md"><DialogHeader><DialogTitle>View Invoice Status</DialogTitle></DialogHeader>
        ...
        <Dialog open={showDialog} onOpenChange={setShowDialog}><DialogContent className="max-w-md"><DialogHeader><DialogTitle>{editing ? "Edit Invoice Status" : "Add Invoice Status"}</DialogTitle><DialogDescription>{editing ? "Update details" : "Create a new record"}</DialogDescription></DialogHeader>
        ...
        <AlertDialog open={showDelete} onOpenChange={setShowDelete}><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Delete Invoice Status</AlertDialogTitle><AlertDialogDescription>Are you sure? This cannot be undone.</AlertDialogDescription></AlertDialogHeader>
