...
interface Row { code: string; description: string | null; entered_by: string | null; entered_on: string | null; modified_by: string | null; modified_on: string | null; }
const TABLE = "tb_batch_status"; const MODULE_NAME = "md_batch_status"; const TITLE = "Batch Status";

const BatchStatusManagement = () => {
  ...
  const saveMutation = useMutation({
    mutationFn: async (r: any) => {
      if (editing) { const { error } = await (supabase as any).from(TABLE).update({ description: r.description, modified_by: userCode, modified_on: new Date().toISOString() }).eq("code", r.code); if (error) throw error; }
      else { const { error } = await (supabase as any).from(TABLE).insert({ ...r, entered_by: userCode, entered_on: new Date().toISOString() }); if (error) throw error; }
    },
  ...
  return (
    <PermissionWrapper moduleName={MODULE_NAME}>
      <div className="container mx-auto p-6 space-y-6">
        <div className="flex items-center justify-between"><div><h1 className="text-3xl font-bold text-foreground">{TITLE}</h1><p className="text-muted-foreground mt-1">Manage batch status master data</p></div>{can("create") && <Button onClick={openAdd}><Plus className="mr-2 h-4 w-4" /> Add Record</Button>}</div>
...
export default BatchStatusManagement;
