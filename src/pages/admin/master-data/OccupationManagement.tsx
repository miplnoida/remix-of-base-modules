...
const MODULE_NAME = "md_occupation";
...
const OccupationManagement = () => {
  ...
  const { data: rows = [], isLoading } = useQuery({
    queryKey: ["tb_occup"],
    queryFn: async () => {
      const { data, error } = await (supabase as any).from("tb_occup").select("*").order("code");
  ...
  const saveMutation = useMutation({
    mutationFn: async (r: Record) => {
      if (editing) {
        const { error } = await (supabase as any).from("tb_occup").update({ short_description: r.short_description, long_description: r.long_description }).eq("code", r.code);
        if (error) throw error;
      } else {
        const { error } = await (supabase as any).from("tb_occup").insert(r);
        if (error) throw error;
      }
    },
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["tb_occup"] }); toast.success(editing ? "Updated" : "Created"); setShowDialog(false); setEditing(null); },
    onError: (e: any) => toast.error("Save failed: " + (e.message || "Unknown error")),
  });

  const deleteMutation = useMutation({
    mutationFn: async (code: string) => { const { error } = await (supabase as any).from("tb_occup").delete().eq("code", code); if (error) throw error; },
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["tb_occup"] }); toast.success("Deleted"); setShowDelete(false); setDeleteId(null); },
    ...
    <PermissionWrapper moduleName={MODULE_NAME}>
      <div className="container mx-auto p-6 space-y-6">
        <div className="flex items-center justify-between">
          <div><h1 className="text-3xl font-bold text-foreground">Occupations</h1><p className="text-muted-foreground mt-1">Manage occupation master data</p></div>
          {can("create") && <Button onClick={openAdd}><Plus className="mr-2 h-4 w-4" /> Add Occupation</Button>}
        </div>
        <Card>
          <CardHeader>
            <CardTitle>Occupations</CardTitle>
            <CardDescription>All occupations in the system</CardDescription>
        ...
        <Dialog open={showView} onOpenChange={setShowView}><DialogContent className="max-w-md"><DialogHeader><DialogTitle>View Occupation</DialogTitle></DialogHeader>
        ...
        <Dialog open={showDialog} onOpenChange={setShowDialog}><DialogContent className="max-w-md"><DialogHeader><DialogTitle>{editing ? "Edit Occupation" : "Add Occupation"}</DialogTitle><DialogDescription>{editing ? "Update details" : "Create a new record"}</DialogDescription></DialogHeader>
        ...
        <AlertDialog open={showDelete} onOpenChange={setShowDelete}><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Delete Occupation</AlertDialogTitle><AlertDialogDescription>Are you sure? This cannot be undone.</AlertDialogDescription></AlertDialogHeader>
