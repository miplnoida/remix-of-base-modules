...
export default function ApprovalMatrixJournal() {
  const { toast } = useToast();
  const [formOpen, setFormOpen] = useState(false);
  const [historyOpen, setHistoryOpen] = useState(false);
  const [selectedMatrix, setSelectedMatrix] = useState<ApprovalMatrix | undefined>();

  const filteredMatrix = approvalMatrix.filter(m => m.processType === "Journal" && m.activeFlag);
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold">Journal Entry Approval Matrix</h1>
          <p className="text-muted-foreground">Configure amount-based journal entry approval workflows</p>
        </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Journal Entry Approval Rules</CardTitle>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
      ...
      <ApprovalMatrixFormDialog
        ...
        onSave={(matrix) => {
          toast({
            ...
            description: `Approval rule for Journal Entry has been ${selectedMatrix ? "updated" : "created"} successfully.`,
          });
        }}
      />
