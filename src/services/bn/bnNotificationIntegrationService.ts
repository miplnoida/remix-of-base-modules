...
export async function notifyClaimCreated(claimId: string, claimNumber: string, benefitType: string, triggeredBy: string) {
  return dispatchBnNotification({
    ...
    entityId: claimId,
    claimId,
    templateData: { ClaimNumber: claimNumber, BenefitType: benefitType, SubmissionDate: new Date().toLocaleDateString() },
    triggeredBy,
  });
}
...
export async function notifyClaimSubmitted(claimId: string, claimNumber: string, benefitType: string, triggeredBy: string) {
  return dispatchBnNotification({
    ...
    entityId: claimId,
    claimId,
    templateData: { ClaimNumber: claimNumber, BenefitType: benefitType, SubmissionDate: new Date().toLocaleDateString(), ExpectedProcessingDays: 10 },
    triggeredBy,
  });
}
...
export async function notifyClaimVerified(claimId: string, claimNumber: string, triggeredBy: string) {
  return dispatchBnNotification({
    ...
    entityId: claimId,
    claimId,
    templateData: { ClaimNumber: claimNumber, VerifiedDate: new Date().toLocaleDateString(), NextStep: 'Benefit calculation and eligibility assessment' },
    triggeredBy,
  });
}
...
export async function notifyCalculationCompleted(
  ...
  return dispatchBnNotification({
    ...
    entityId: claimId,
    claimId,
    templateData: { ClaimNumber: claimNumber, BenefitType: benefitType, WeeklyRate: weeklyRate, MonthlyRate: monthlyRate, LumpSum: lumpSum, CalcDate: new Date().toLocaleDateString(), OverrideApplied: overrideApplied },
    triggeredBy,
...
export async function notifyPayableBlocked(instructionId: string, claimId: string, claimNumber: string, claimantName: string, amount: number, blockReason: string, triggeredBy: string) {
  return dispatchBnNotification({
    ...
    claimId,
    templateData: { ClaimNumber: claimNumber, ClaimantName: claimantName, Amount: amount, BlockReason: blockReason, BlockedDate: new Date().toLocaleDateString(), InstructionId: instructionId },
    triggeredBy,
  });
}
...
export async function notifyPayableReady(instructionId: string, claimNumber: string, amount: number, paymentMethod: string, frequency: string, triggeredBy: string) {
  return dispatchBnNotification({
    eventCode: 'bn.payable.ready',
    entityId: instructionId,
    templateData: { ClaimNumber: claimNumber, Amount: amount, PaymentMethod: paymentMethod, Frequency: frequency, ReadyDate: new Date().toLocaleDateString() },
    triggeredBy,
  });
}
...
export async function notifyBatchCreated(batchId: string, batchNumber: string, batchType: string, instructionCount: number, totalAmount: number, triggeredBy: string) {
  return dispatchBnNotification({
    eventCode: 'bn.batch.created',
    entityId: batchId,
    templateData: { BatchNumber: batchNumber, BatchType: batchType, CreatedDate: new Date().toLocaleDateString(), InstructionCount: instructionCount, TotalAmount: totalAmount },
    triggeredBy,
  });
}
...
export async function notifyBatchApproved(batchId: string, batchNumber: string, approvedBy: string, instructionCount: number, totalAmount: number, paymentMethod: string, triggeredBy: string) {
  return dispatchBnNotification({
    eventCode: 'bn.batch.approved',
    entityId: batchId,
    templateData: { BatchNumber: batchNumber, ApprovedBy: approvedBy, ApprovedDate: new Date().toLocaleDateString(), InstructionCount: instructionCount, TotalAmount: totalAmount, PaymentMethod: paymentMethod },
    triggeredBy,
  });
}
...
export async function notifyIssueStarted(batchId: string, batchNumber: string, instructionCount: number, totalAmount: number, paymentMethod: string, triggeredBy: string) {
  return dispatchBnNotification({
    eventCode: 'bn.issue.started',
    entityId: batchId,
    templateData: { BatchNumber: batchNumber, InstructionCount: instructionCount, TotalAmount: totalAmount, IssueStartTime: new Date().toLocaleString(), PaymentMethod: paymentMethod },
    triggeredBy,
  });
}
...
export async function notifyIssueCompleted(batchId: string, batchNumber: string, successCount: number, totalAmount: number, triggeredBy: string) {
  return dispatchBnNotification({
    eventCode: 'bn.issue.completed',
    entityId: batchId,
    templateData: { BatchNumber: batchNumber, SuccessCount: successCount, TotalAmount: totalAmount, IssueDate: new Date().toLocaleDateString() },
    triggeredBy,
  });
}
...
export async function notifyIssueFailed(batchId: string, batchNumber: string, failedCount: number, successCount: number, errorSummary: string, triggeredBy: string) {
  return dispatchBnNotification({
    eventCode: 'bn.issue.failed',
    entityId: batchId,
    templateData: { BatchNumber: batchNumber, FailedCount: failedCount, SuccessCount: successCount, ErrorSummary: errorSummary, IssueDate: new Date().toLocaleDateString() },
    triggeredBy,
  });
}
...
export async function notifyPostIssueCompleted(claimId: string, claimNumber: string, tasksCompleted: number, claimStatus: string, triggeredBy: string) {
  return dispatchBnNotification({
    ...
    entityId: claimId,
    claimId,
    templateData: { ClaimNumber: claimNumber, TasksCompleted: tasksCompleted, CompletionDate: new Date().toLocaleDateString(), ClaimStatus: claimStatus },
    triggeredBy,
  });
}
...
export async function notifyCancellationRequested(instructionId: string, claimId: string, claimNumber: string, amount: number, cancellationReason: string, requestedBy: string, originalChequeNumber: string, triggeredBy: string) {
  return dispatchBnNotification({
    ...
    entityId: instructionId,
    claimId,
    templateData: { ClaimNumber: claimNumber, Amount: amount, CancellationReason: cancellationReason, RequestedBy: requestedBy, RequestDate: new Date().toLocaleDateString(), OriginalChequeNumber: originalChequeNumber },
    triggeredBy,
  });
}
