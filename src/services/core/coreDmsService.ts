...
export const coreDmsService = {
  ...
  async streamByLink(linkId: string, action: "stream" | "download" = "stream", fileName?: string): Promise<string> {
    ...
    const url = `${(supabase as any).supabaseUrl ?? import.meta.env.VITE_SUPABASE_URL}/functions/v1/document-proxy`;
    const resp = await fetch(url, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${accessToken}`,
        apikey: (supabase as any).supabaseKey ?? import.meta.env.VITE_SUPABASE_PUBLISHABLE_KEY,
