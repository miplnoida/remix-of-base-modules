...
export const BN_MORTALITY_COMMANDS: readonly BnMortalityCommandSpec[] = [
  ...
  { command: 'BN_MORTALITY_MATCH_PERSON',                 capability: 'bn_mortality:match_person',     requiresMakerChecker: false, transactional: true,  requiresJustification: false, implemented: true },
  ...
  { command: 'BN_MORTALITY_ASSIGN',                       capability: 'bn_mortality:assign',           requiresMakerChecker: false, transactional: false, requiresJustification: false, implemented: true },
  ...
  { command: 'BN_MORTALITY_PLACE_PROVISIONAL_HOLD',       capability: 'bn_mortality:decide',           requiresMakerChecker: false, transactional: true,  requiresJustification: true,  implemented: false, blocker: 'BN-MORT-2B.1 §4 — Award/Payment servicing suspension boundary not invoked; only mortality-side status changes.' },
  { command: 'BN_MORTALITY_RELEASE_HOLD',                 capability: 'bn_mortality:release_hold',     requiresMakerChecker: false, transactional: true,  requiresJustification: true,  implemented: false, blocker: 'BN-MORT-2B.1 §4 — Award/Payment servicing reversal boundary not invoked.' },
  ...
  { command: 'BN_MORTALITY_RESOLVE_CONFLICT',             capability: 'bn_mortality:resolve_conflict', requiresMakerChecker: false, transactional: false, requiresJustification: true,  implemented: true },
  ...
  { command: 'BN_MORTALITY_PREPARE_IMPACT',               capability: 'bn_mortality:prepare_impact',   requiresMakerChecker: false, transactional: true,  requiresJustification: false, implemented: false, blocker: 'BN-MORT-2B.1 §3 — Real person→award→schedule scan and PAD exposure calculation pending; current handler only transitions state.' },
  { command: 'BN_MORTALITY_SUBMIT_IMPACT',                capability: 'bn_mortality:submit_impact',    requiresMakerChecker: false, transactional: false, requiresJustification: false, implemented: true },
  { command: 'BN_MORTALITY_RETURN_IMPACT',                capability: 'bn_mortality:return_impact',    requiresMakerChecker: false, transactional: false, requiresJustification: true,  implemented: true },
  ...
  { command: 'BN_MORTALITY_COMPLETE_FOLLOWON',            capability: 'bn_mortality:complete_followon', requiresMakerChecker: false, transactional: false, requiresJustification: false, implemented: false, blocker: 'BN-MORT-2B.1 §9 — Impact/referral completion gate not enforced.' },
