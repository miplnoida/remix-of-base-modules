...
BEGIN
  ...
  -- Uses payment_sequence_no::TEXT for cn_payment (matching get_batch_cheques_for_verification)
  SELECT COALESCE(SUM(
  ...
  -- Count unverified cheques (only from active-receipt transactions)
  ...
  LOOP
    IF v_system.mop_code = 'CSH' THEN
      -- Include opening balance (offset_amount) in CSH system total
