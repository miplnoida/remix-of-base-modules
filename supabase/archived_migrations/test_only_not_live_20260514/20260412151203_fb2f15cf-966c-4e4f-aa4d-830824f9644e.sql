...
-- Fix C3 payment failures: timestamp cast + overpayment guard sequence_no filter
...
BEGIN
  ...
  LOOP
    ...
    IF v_guard_period_ts IS NOT NULL AND v_guard_seq IS NOT NULL THEN
      -- Get original amount from C3 reported
      SELECT
        CASE v_guard_code
      ...
      -- FIX Bug 2: Query c3_payment_components (which has sequence_no) instead of cn_payment
      -- This prevents cross-sequence payment inflation
  ...
  LOOP
    ...
    -- FIX Bug 1: Cast period to timestamp to prevent type mismatch error
    INSERT INTO public.cn_payment (
      payment_id, payment_code, fund_code, payment_amount, payment_date,
      mop_code, mop_number, bank_code, bank_lodgement_code,
      mop_transit_number, mop_account_number, mop_notes1,
      credit_card_code, expiration_date, cheque_date, period,
      base_currency, currency_conversion_rate, card_machine_id
    ) VALUES (
      v_payment_id,
      v_meth->>'payment_code',
      v_meth->>'fund_code',
      (v_meth->>'amount')::numeric,
      p_date_received::timestamp,
      v_meth->>'mop_code',
      v_meth->>'mop_number',
      v_meth->>'bank_code',
      v_meth->>'bank_lodgement_code',
      v_meth->>'mop_transit_number',
      v_meth->>'mop_account_number',
      v_meth->>'mop_notes1',
      v_meth->>'credit_card_code',
      v_exp_normalized::timestamp,
      CASE WHEN v_meth->>'cheque_date' IS NOT NULL AND v_meth->>'cheque_date' != ''
           THEN (v_meth->>'cheque_date')::timestamp
           ELSE NULL END,
