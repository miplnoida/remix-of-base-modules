...
-- Restore the proven working calculate_c3_contributions RPC from migration 20260303200138
-- with SECURITY DEFINER, configurable week_start_day, and configurable filing/penalty logic
...
DECLARE
  ...
  v_slab_code VARCHAR;
  -- Configurable filing/penalty fields
  v_week_start_day INTEGER;
  v_filing_window_unit INTEGER;  -- 1=Months, 0=Days
  ...
  v_pg_dow INTEGER;  -- PostgreSQL DOW for configurable week start
BEGIN
  ...
  -- ===== Read configurable filing/penalty settings =====
  ...
  -- Map week_start_day (1=Mon..7=Sun) to PostgreSQL DOW (0=Sun,1=Mon..6=Sat)
  IF v_week_start_day = 7 THEN
    v_pg_dow := 0;  -- Sunday
  ELSE
    v_pg_dow := v_week_start_day;  -- 1=Mon..6=Sat maps directly
  END IF;

  -- ===== Compute week-start days in the period month =====
  v_mondays := ARRAY[]::DATE[];
  v_monday := v_period_date;
  -- Find first occurrence of the configured week start day
  ...
  -- Compute filing deadline based on configurable window
  IF v_filing_window_unit = 1 THEN
    -- Months-based filing window
    v_filing_deadline := (v_due_date + (v_filing_window_value || ' months')::INTERVAL)::DATE;
  ELSE
    -- Days-based filing window
  ...
  -- Compute delay periods for penalty calculation
  IF v_filing_window_unit = 1 THEN
    -- Months-based: count calendar months late from filing deadline
    v_delay_periods := v_months_late_calendar;
  ELSE
    -- Days-based: count 30-day periods late from filing deadline
    v_delay_periods := v_months_late;
  END IF;
  
  -- Split delay into initial and subsequent penalty periods
  ...
  -- No cap on subsequent periods - they continue indefinitely until C3 is submitted
  ...
  LOOP
    ...
    ELSE
      v_employee_ss := 0;
      v_employer_ss := 0;
      -- EIB is still calculated for age-exempt employees
      v_employer_eib_base := v_taxable_wages;
      ...
      IF v_hp_eligible AND v_hp_ssc_include AND v_hp_ssc_eib THEN
        -- holiday already in taxable_wages, keep it
        NULL;
    ...
    IF NOT v_is_age_exempt_levy THEN
      ...
      -- ===== HOLIDAY LEVY (separate calculation) =====
  ...
  -- ===== PENALTIES (configurable filing/penalty logic) =====
