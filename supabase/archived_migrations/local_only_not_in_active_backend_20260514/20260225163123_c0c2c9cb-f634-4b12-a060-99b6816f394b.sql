...
-- ============================================================
-- Add columns to ip_application_documents for consolidation
-- These columns currently only exist on ip_documents
-- ============================================================
...
-- Index for fast lookups by application reference
CREATE INDEX IF NOT EXISTS idx_ip_app_docs_app_ref
  ON ip_application_documents(application_reference_number);

-- ============================================================
-- Update convert_application_atomic RPC to populate
-- application_reference_number on document INSERT
-- ============================================================
...
BEGIN
  ...
  -- Insert documents (NOW WITH application_reference_number)
  IF p_documents IS NOT NULL AND jsonb_array_length(p_documents) > 0 THEN
    FOR v_doc IN SELECT * FROM jsonb_array_elements(p_documents) LOOP
      ...
      INSERT INTO ip_application_documents (
        ssn, application_reference_number, document_name, document_type, file_name, file_path, url, signed_url, mime_type, file_size, uploaded_at, source_document_id, metadata, created_by
      )
      VALUES (
        v_final_ssn,
