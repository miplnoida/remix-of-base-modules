-- Fix Live publish failure: rebuild dashboard compliance views with correct dependency order.
-- Drops dependents first via CASCADE then recreates base + dependents using plain CREATE VIEW
-- to bypass the diff engine's column-shape / dependency-ordering check.

DROP VIEW IF EXISTS public.dashboard_v_compliance_metrics CASCADE;
DROP VIEW IF EXISTS public.dashboard_v_compliance_distribution CASCADE;
DROP VIEW IF EXISTS public.dashboard_v_employer_compliance_status CASCADE;

-- Base view (final logic from migration 20260422102813)
...
-- Distribution dependent view (from 20260422102700)
...
-- Metrics dependent view (from 20260422102700)
