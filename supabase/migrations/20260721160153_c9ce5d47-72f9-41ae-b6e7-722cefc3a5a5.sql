...
DECLARE
  ...
  v_content_hash text;
  v_first_to text;
  v_system_tokens jsonb;
  v_request_tokens jsonb;
  v_recipient_tokens jsonb := '{}'::jsonb;
...
BEGIN
  ...
  -- Namespaced token bags (used by the contract resolver below).
  ...
  -- Active default scenario (event-owned business tokens only).
  ...
  -- Baseline flat token bag (existing behaviour).
  ...
  -- Contract-aware augmentation: for each declared variable missing from
  -- v_tokens, resolve from its declared source + canonical path.
  FOR c IN
  ...
  LOOP
    IF v_tokens ? c.variable_name THEN
      CONTINUE;
    END IF;
    ...
    ELSIF c.source_type = 'event_payload' THEN
      -- nested path lookup against scenario tokens (canonical form)
      v_path_parts := string_to_array(c.canonical_path,'.');
      BEGIN
        v_val := v_scenario_tokens #> v_path_parts;
      EXCEPTION WHEN OTHERS THEN v_val := NULL; END;
      -- flat-key compatibility fallback: variable_name key in scenario
    ...
    IF v_val IS NOT NULL AND v_val <> 'null'::jsonb THEN
      -- Emit as a plain string for the flat renderer (strip JSON quoting).
  ...
  INSERT INTO public.communication_preview_snapshot(
    ...
    v_content_hash, encode(digest(v_tokens::text,'sha256'),'hex'),
    v_unresolved,
    ...
    'PREPARED', now() + interval '24 hours', now(), now()
