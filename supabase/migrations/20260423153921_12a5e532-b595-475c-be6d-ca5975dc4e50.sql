-- Non-destructive alignment of compliance dashboard views.
-- Uses CREATE OR REPLACE VIEW only (no DROP, no CASCADE) so it works
-- whether the views are missing (Live) or already present (Test).
