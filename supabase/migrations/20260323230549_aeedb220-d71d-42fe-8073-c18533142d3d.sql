...
-- Drop both overloads of close_batch
DROP FUNCTION IF EXISTS public.close_batch(text, text);
DROP FUNCTION IF EXISTS public.close_batch(character varying, character varying);

-- Recreate single close_batch with fixed CSH join (tb_currencies directly)
...
BEGIN
  ...
  -- 2. Physical CSH: cn_cash_count -> denominations -> tb_currencies DIRECTLY
  SELECT COALESCE(SUM(
  ...
  JOIN public.tb_currencies tc ON tc.id = cc.currency_id
  ...
  -- 6. Compare rounded totals (only physical MOPs)
