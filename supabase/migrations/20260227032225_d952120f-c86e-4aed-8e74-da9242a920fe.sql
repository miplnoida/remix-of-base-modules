...
DECLARE
  ...
  -- Slab code to use for each payment
  v_slab_code VARCHAR;
BEGIN
  ...
  LOOP
    ...
    -- ===== EMPLOYEE LEVY (per-payment slab-based) =====
    v_employee_levy := 0;
    v_bonus_levy := 0;
    ...
    IF NOT v_is_age_exempt_levy THEN
      ...
      -- Determine if monthly ceiling override applies
      ...
      IF v_use_monthly_override THEN
        -- MONTHLY CEILING OVERRIDE: use M slabs on total monthly amount
        v_week_levy := 0;
        ...
        END IF;
      ELSE
        -- STANDARD: per-payment slab calculation
        -- For Weekly, Bi-Weekly, 2-Monthly: apply matching slab to EACH individual amount
        -- For Monthly: apply M slab to the total
        
        IF v_pay_period_code = 'M' THEN
          -- Monthly: one payment = total of weeks + holiday → M slab
          ...
          END IF;
        ELSE
          -- Weekly / Bi-Weekly / 2-Monthly: apply pay-period slab to EACH individual amount
          -- v_slab_code matches the pay period: W, E2W, or 2M
          v_slab_code := v_pay_period_code;
          
          -- Build array of all individual payment amounts (w1-w5 + holiday)
  ...
  -- Bases per business rules:
  --   Levy penalty base = employee levy + employer levy (severance excluded)
