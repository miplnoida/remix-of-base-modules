-- CH-GL-02 Slice D — align check_comm_hub_readiness with the real schema of
-- communication_hub_event_live_control (which has `status` in
-- {'dry_run_only','live_manual_only','live_cron_allowed'} and no channel column).
...
'CH-GL-02 Slice D: Go Live readiness aggregator. Read-only. Uses communication_hub_event_live_control.status (no channel column) for event-certification checks.';