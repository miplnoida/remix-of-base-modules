...
-- =====================================================
-- Holiday Pay Policy Resolution & Pending Pay RPCs
-- =====================================================

-- 1) Resolve holiday pay policy for a given period + policy_type
CREATE OR REPLACE FUNCTION public.resolve_holiday_pay_policy(
  p_period_date DATE,
  p_period_month INTEGER,  -- 1-indexed
...
BEGIN
  -- Try exception first (month/year match, override_default)
  SELECT jsonb_build_object(
    'source', 'exception',
    'id', id,
    'policy_type', policy_type,
    'levy_include', COALESCE(levy_include, false),
    'levy_calculation_method', COALESCE(levy_calculation_method, 'merge'),
    'levy_calc_flat_enabled', COALESCE(levy_calc_flat_enabled, false),
    'levy_calc_flat_percentage', levy_calc_flat_percentage,
    'levy_calc_slab_enabled', COALESCE(levy_calc_slab_enabled, false),
    'levy_distribution', levy_distribution,
    'ssc_include', COALESCE(ssc_include, false),
    'ssc_contrib_employee', COALESCE(ssc_contrib_employee, false),
    'ssc_contrib_employer', COALESCE(ssc_contrib_employer, false),
    'ssc_contrib_eib', COALESCE(ssc_contrib_eib, false),
    'distribution_enabled', COALESCE(distribution_enabled, true),
    'include_in_severance', COALESCE(include_in_severance, false),
    'min_holiday_amount', min_holiday_amount,
    'max_holiday_amount', max_holiday_amount
  ) INTO v_result
  FROM public.c3_holiday_pay_policy_exceptions
  WHERE is_active = true
    AND override_default = true
    AND policy_type = p_policy_type
    AND exception_month = p_period_month
  ...
  FROM public.c3_holiday_pay_policy_default
  WHERE is_active = true
    AND policy_type = p_policy_type
    AND p_period_date >= date_from
    AND (date_to IS NULL OR p_period_date <= date_to)
  ORDER BY date_from DESC
  LIMIT 1;
...
-- 2) Fetch pending holiday pay for a given SSN and period
...
BEGIN
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    ...
    'ssn', ssn,
    'amount', amount,
    'source_c3_period', source_c3_period,
...
-- 3) Mark pending holiday pay as applied
CREATE OR REPLACE FUNCTION public.apply_pending_holiday_pay(
  p_pending_ids UUID[],
...
AS $$
BEGIN
  UPDATE public.c3_pending_holiday_pay
  SET status = 'applied',
      applied_on = NOW(),
      modified_by = p_applied_by,
      modified_on = NOW()
...
GRANT EXECUTE ON FUNCTION public.apply_pending_holiday_pay(UUID[], TEXT) TO authenticated;

-- 4) Create pending holiday pay record (for future period distribution)
CREATE OR REPLACE FUNCTION public.create_pending_holiday_pay(
  p_ssn TEXT,
  p_amount NUMERIC,
  p_source_c3_period TEXT,
  p_target_year INTEGER,
  p_target_month INTEGER,
  p_holiday_date_from DATE DEFAULT NULL,
  p_holiday_date_to DATE DEFAULT NULL,
...
DECLARE
  v_id UUID;
BEGIN
  INSERT INTO public.c3_pending_holiday_pay (
    ssn, amount, source_c3_period, target_year, target_month,
    holiday_date_from, holiday_date_to, status, created_by
  ) VALUES (
    p_ssn, p_amount, p_source_c3_period, p_target_year, p_target_month,
    p_holiday_date_from, p_holiday_date_to, 'pending', p_created_by
  )
...
GRANT EXECUTE ON FUNCTION public.create_pending_holiday_pay(TEXT, NUMERIC, TEXT, INTEGER, INTEGER, DATE, DATE, TEXT) TO authenticated;

-- 5) Update calculate_c3_contributions to include holiday pay policy logic
...
BEGIN
  ...
  LOOP
    ...
    -- Determine if holiday has dates provided
    ...
    -- ===== SS CALCULATIONS (with bonus + holiday SSC contribution adjustments) =====
    ...
    -- Holiday SSC: add to employee SS base if policy says so
    IF v_hp_eligible AND v_hp_ssc_include AND v_hp_ssc_employee THEN
      -- Holiday is already in taxable_wages, no adjustment needed for employee
      NULL;
    ELSIF NOT v_hp_eligible OR NOT v_hp_ssc_include OR NOT v_hp_ssc_employee THEN
      -- Remove holiday from employee SS base (it's already in taxable_wages via total_wages - bonus)
      -- Actually holiday IS in taxable_wages. If SSC should NOT include holiday, subtract it.
    ...
    IF NOT v_is_age_exempt_ss THEN
      ...
      -- Holiday SSC employer adjustment
    ...
    -- ===== EMPLOYEE LEVY (with bonus policy + holiday pay levy policy) =====
    v_employee_levy := 0;
    v_bonus_levy := 0;
    v_holiday_levy := 0;
    
    IF NOT v_is_age_exempt_levy THEN
      -- Determine base levy amounts: wages excluding holiday if holiday levy is handled separately
      v_monthly_total_for_levy := v_week1 + v_week2 + v_week3 + v_week4 + v_week5;
      
      -- Include holiday in base levy only if: no holiday policy, OR holiday levy uses merge method
      ...
      ELSIF NOT v_hp_eligible OR NOT v_hp_levy_include THEN
        -- Standard: holiday is in the regular levy base (original behavior)
        v_monthly_total_for_levy := v_monthly_total_for_levy + v_holiday;
      END IF;
      -- If holiday levy is 'separate', don't add holiday to v_monthly_total_for_levy
      
      IF v_bonus_eligible AND v_bonus_include_in_levy AND v_bp_calculation_method = 'merge' THEN
        -- ===== MERGE BONUS: distribute bonus into wage slots, then apply slab =====
        v_merged_amounts := ARRAY[v_week1, v_week2, v_week3, v_week4, v_week5, v_holiday];
        
        -- Remove holiday from merged amounts if holiday levy is separate
        IF v_hp_eligible AND v_hp_levy_include AND v_hp_levy_method = 'separate' THEN
          v_merged_amounts[6] := 0; -- Don't include holiday in merged levy calc
        END IF;
      ...
      ELSIF v_bonus_eligible AND v_bonus_include_in_levy AND v_bp_calculation_method = 'separate' THEN
        -- ===== SEPARATE BONUS: regular levy on wages, then bonus levy separately =====
        -- Adjust monthly total for holiday if holiday levy is separate
        IF v_hp_eligible AND v_hp_levy_include AND v_hp_levy_method = 'separate' THEN
          v_monthly_total_for_levy := v_monthly_total_for_levy - v_holiday;
        END IF;
        ...
        ELSE
          ...
          -- Add holiday to slab array only if not separate
      ...
      ELSE
        ...
        -- If holiday levy is separate, remove holiday from standard calc
        IF v_hp_eligible AND v_hp_levy_include AND v_hp_levy_method = 'separate' THEN
          v_monthly_total_for_levy := v_monthly_total_for_levy - v_holiday;
        END IF;
        ...
        ELSE
          v_slab_code := v_pay_period_code;
          v_amounts_array := ARRAY[v_week1, v_week2, v_week3, v_week4, v_week5];
          -- Add holiday only if not being handled separately
          IF NOT (v_hp_eligible AND v_hp_levy_include AND v_hp_levy_method = 'separate') THEN
            v_amounts_array := v_amounts_array || v_holiday;
          END IF;
          FOR v_i IN 1..array_length(v_amounts_array, 1) LOOP
      ...
      -- ===== HOLIDAY LEVY (separate calculation if holiday policy uses 'separate') =====
    ...
    -- Employer levy (adjust base if bonus included + holiday policy)
    ...
    END IF;
    -- Holiday: if levy not included, remove from employer levy base
    ...
    -- Employer severance (adjust base if bonus + holiday included)
  ...
  -- ===== PENALTIES (using post-bonus/holiday contribution sums) =====
