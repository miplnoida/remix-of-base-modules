...
BEGIN
  ...
  -- FIX: core_template_version columns are subject / body_html / body_text
  -- (not *_template). Using the wrong names crashed the RPC with
  -- "record v_ver has no field subject_template".
  v_render_subject := public.render_comm_hub_template(v_ver.subject, v_tokens);
  v_render_html    := public.render_comm_hub_template(v_ver.body_html, v_tokens);
  v_render_text    := public.render_comm_hub_template(v_ver.body_text, v_tokens);
