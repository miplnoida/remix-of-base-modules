-- Fix ambiguous overloads for schedule_meeting RPC (PostgREST cannot choose)
-- Keep the canonical signature that uses the meeting_type enum, and drop the text-based overload.
...
BEGIN
  ...
  INSERT INTO meetings (
    ...
    p_user_name,
    CASE
      WHEN p_user_name IS NULL THEN NULL
      ELSE LEFT(p_user_name, 10)
    END
  )
