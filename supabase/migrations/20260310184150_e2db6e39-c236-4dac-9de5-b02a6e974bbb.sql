...
DECLARE
  ...
  -- Holiday target slot for levy merge (NEW)
...
BEGIN
  ...
  -- ===== Compute Mondays in the period month =====
  v_mondays := ARRAY[]::DATE[];
  v_monday := v_period_date;
  WHILE EXTRACT(DOW FROM v_monday) != 1 LOOP
    v_monday := v_monday + 1;
  END LOOP;
  ...
  LOOP
    ...
    -- Determine pay period code early (needed for monthly holiday clipping)
    ...
    IF v_holiday_has_dates THEN
      v_hp_policy_type := 'with_dates';
    ELSE
      v_hp_policy_type := 'without_dates';
    END IF;
    ...
    IF v_holiday > 0 AND v_holiday_has_dates AND v_monday_count > 0 THEN
      ...
      -- ===== MONTHLY PAY PERIOD: CLIP HOLIDAY DATES TO CURRENT MONTH =====
      IF v_pay_period_code = 'M' THEN
        v_month_start := v_period_date;
        v_month_end := (v_period_date + INTERVAL '1 month' - INTERVAL '1 day')::DATE;
        
        v_total_holiday_days := (v_hol_end - v_hol_start) + 1;
        
        IF v_hol_start < v_month_start OR v_hol_end > v_month_end THEN
          DECLARE
            v_clipped_start DATE;
            v_clipped_end DATE;
          BEGIN
            v_clipped_start := GREATEST(v_hol_start, v_month_start);
            v_clipped_end := LEAST(v_hol_end, v_month_end);
            
            IF v_clipped_start > v_clipped_end THEN
              v_holiday := 0;
              v_hol_start := v_month_start;
              v_hol_end := v_month_start;
            ELSE
              v_in_month_days := (v_clipped_end - v_clipped_start) + 1;
              v_holiday := ROUND(v_original_holiday * v_in_month_days::NUMERIC / v_total_holiday_days::NUMERIC, 2);
              v_hol_start := v_clipped_start;
              v_hol_end := v_clipped_end;
            END IF;
          END;
        END IF;
      END IF;
      ...
      IF v_hp_distribution_enabled AND v_holiday > 0 THEN
        ...
        FOR v_i IN 1..v_monday_count LOOP
          ...
          ELSE
            v_week_end := (v_period_date + INTERVAL '1 month' - INTERVAL '1 day')::DATE;
          END IF;
          
          IF v_hol_start <= v_week_end AND v_hol_end >= v_week_start THEN
            v_overlap_days[v_i] := GREATEST(0, 
              (LEAST(v_hol_end, v_week_end) - GREATEST(v_hol_start, v_week_start)) + 1
            );
        ...
        IF v_total_overlap > 0 THEN
          ...
          -- Fix rounding: adjust last distributed week to absorb remainder
          DECLARE
          ...
          -- ===== CHANGE 2: E2W/2M PAYMENT PERIOD BOUNDARY AGGREGATION =====
          -- For Bi-Weekly and Semi-Monthly, aggregate distributed amounts into payment period target weeks.
          -- Period 1 = weeks 1+2 → target week 2; Period 2 = weeks 3+4 → target week 4.
          -- Weekly: each week is its own period, no aggregation needed.
          -- Monthly: already clipped to month, no aggregation needed.
          IF v_pay_period_code IN ('E2W', '2M') THEN
            v_period1_amount := v_hol_distributed[1] + v_hol_distributed[2];
            v_period2_amount := v_hol_distributed[3] + v_hol_distributed[4];
            v_hol_distributed[1] := 0;
            v_hol_distributed[2] := v_period1_amount;
            v_hol_distributed[3] := 0;
            v_hol_distributed[4] := v_period2_amount;
            -- week 5 stays as-is (rare months with 5 Mondays)
          END IF;
          
          -- Add distributed amounts to weekly slots
          ...
          -- Zero out standalone holiday to prevent double-counting
          v_holiday := 0;
          
          v_holiday_distribution := jsonb_build_object(
            'distributed', true,
            'week1', v_hol_distributed[1],
            'week2', v_hol_distributed[2],
            'week3', v_hol_distributed[3],
            'week4', v_hol_distributed[4],
            ...
            'holidayStart', v_hol_start,
            'holidayEnd', v_hol_end,
            'periodBoundaryAggregated', v_pay_period_code IN ('E2W', '2M'),
            'monthlyClipped', v_pay_period_code = 'M' AND v_original_holiday != (v_hol_distributed[1] + v_hol_distributed[2] + v_hol_distributed[3] + v_hol_distributed[4] + v_hol_distributed[5]),
            'originalHolidayAmount', v_original_holiday,
            'proratedHolidayAmount', CASE WHEN v_pay_period_code = 'M' AND v_original_holiday != (v_hol_distributed[1] + v_hol_distributed[2] + v_hol_distributed[3] + v_hol_distributed[4] + v_hol_distributed[5]) THEN v_hol_distributed[1] + v_hol_distributed[2] + v_hol_distributed[3] + v_hol_distributed[4] + v_hol_distributed[5] ELSE NULL END
          );
        END IF;
      END IF;
    END IF;
    ...
    -- ===== HOLIDAY PAY POLICY RESOLUTION (for non-distributed holiday) =====
    IF v_holiday_distribution IS NULL THEN
      v_hp_policy := NULL;
    END IF;
    v_holiday_levy := 0;
    ...
    -- When holiday was distributed into weeks, suppress separate policy rules
    ...
    -- ===== CHANGE 1: COMPUTE HOLIDAY TARGET SLOT FOR LEVY MERGE =====
    -- Determine which weekly slot the undistributed holiday should be merged into
    -- based on the policy's levy_distribution configuration.
    IF v_holiday > 0 AND v_hp_levy_distribution IS NOT NULL THEN
      CASE v_pay_period_code
        WHEN 'W' THEN
          IF COALESCE((v_hp_levy_distribution->'weekly'->>'w1')::boolean, false) THEN v_hp_target_slot := 1;
          ...
          ELSIF COALESCE((v_hp_levy_distribution->'weekly'->>'w4')::boolean, false) THEN v_hp_target_slot := 4;
          ELSE v_hp_target_slot := 4;
          END IF;
        WHEN 'E2W' THEN
          IF COALESCE((v_hp_levy_distribution->'biweekly'->>'b1')::boolean, false) THEN v_hp_target_slot := 2;
          ELSIF COALESCE((v_hp_levy_distribution->'biweekly'->>'b2')::boolean, false) THEN v_hp_target_slot := 4;
          ELSE v_hp_target_slot := 4;
          END IF;
        WHEN '2M' THEN
          IF COALESCE((v_hp_levy_distribution->'semimonthly'->>'s1')::boolean, false) THEN v_hp_target_slot := 2;
          ELSIF COALESCE((v_hp_levy_distribution->'semimonthly'->>'s2')::boolean, false) THEN v_hp_target_slot := 4;
          ELSE v_hp_target_slot := 4;
          END IF;
        WHEN 'M' THEN
          v_hp_target_slot := 1;
    ...
    ELSIF v_holiday > 0 THEN
      -- No policy distribution configured, use pay-period defaults
      v_hp_target_slot := CASE v_pay_period_code
        WHEN 'W' THEN 4
        WHEN 'E2W' THEN 4
        WHEN '2M' THEN 4
        ELSE 1
    ...
    -- Build metadata for without_dates policy merge
    IF v_holiday > 0 AND v_holiday_distribution IS NULL AND v_hp_target_slot > 0
       AND NOT (v_hp_eligible AND v_hp_levy_include AND v_hp_levy_method = 'separate') THEN
      v_holiday_distribution := jsonb_build_object(
        'distributed', false,
        'policyMerged', true,
        'targetSlot', v_hp_target_slot,
        'holidayAmount', v_holiday,
        'payPeriod', v_pay_period_code,
        'policyType', v_hp_policy_type
      );
    END IF;
    ...
    IF NOT v_is_age_exempt_levy THEN
      ...
      IF v_bonus_eligible AND v_bonus_include_in_levy AND v_bp_calculation_method = 'merge' THEN
        -- ===== MERGE BONUS =====
        -- CHANGE 3: Build merged_amounts with holiday in target slot instead of standalone 6th element
        v_merged_amounts := ARRAY[v_week1, v_week2, v_week3, v_week4, v_week5, 0];
        
        -- Place undistributed holiday into policy target slot for levy evaluation
        IF v_holiday > 0 AND NOT (v_hp_eligible AND v_hp_levy_include AND v_hp_levy_method = 'separate') THEN
      ...
      ELSIF v_bonus_eligible AND v_bonus_include_in_levy AND v_bp_calculation_method = 'separate' THEN
        ...
        ELSIF v_pay_period_code = 'M' THEN
          ...
          END IF;
        ELSE
          -- CHANGE 3: Place holiday in target slot instead of appending as standalone element
          ...
          IF v_holiday > 0 AND NOT (v_hp_eligible AND v_hp_levy_include AND v_hp_levy_method = 'separate') THEN
            ...
            END IF;
          END IF;
          FOR v_i IN 1..array_length(v_amounts_array, 1) LOOP
