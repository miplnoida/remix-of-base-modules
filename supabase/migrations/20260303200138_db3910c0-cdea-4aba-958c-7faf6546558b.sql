...
-- Replace calculate_c3_contributions with holiday-pay date-based distribution
-- Holiday pay is distributed into weekly slots BEFORE bonus/levy/SS calculations.
-- Distribution is proportional to the number of holiday days overlapping each payroll week.
-- After distribution, v_holiday is zeroed to prevent double-counting.
-- Re-running produces identical results (idempotent: holiday is always read from the raw input and distributed fresh).
...
BEGIN
  ...
  LOOP
    ...
    -- When dates are provided, distribute holiday pay into weekly slots proportionally
    -- based on how many holiday days fall into each payroll week.
    -- This happens BEFORE any bonus/levy/SS calculations.
    ...
    IF v_holiday > 0 AND v_holiday_has_dates AND v_monday_count > 0 THEN
      ...
      -- Resolve policy to check if distribution is enabled
      ...
      IF v_hp_distribution_enabled THEN
        -- Calculate overlap days for each payroll week
        ...
        FOR v_i IN 1..v_monday_count LOOP
          ...
          IF v_i < v_monday_count THEN
            v_week_end := v_mondays[v_i + 1] - 1;  -- day before next Monday
          ELSE
            -- Last week extends to end of month
            v_week_end := (v_period_date + INTERVAL '1 month' - INTERVAL '1 day')::DATE;
          END IF;
          
          -- Overlap = max(0, min(hol_end, week_end) - max(hol_start, week_start) + 1)
    ...
    -- ===== HOLIDAY PAY POLICY RESOLUTION (for non-distributed holiday, i.e. without_dates) =====
    ...
    -- When holiday was distributed into weeks, policy rules for levy/SSC are suppressed
    -- (the distributed amounts are already in the weekly wage base and calculated normally)
    ...
    -- Holiday pay capping (only for non-distributed)
