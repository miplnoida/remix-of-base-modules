...
-- Fix levy penalty base calculation to include severance amount
-- Levy penalty base = employee_levy + employer_levy + severance_amount
-- Severance penalty base = severance_amount only (already correct)
-- SS fine base = employee_ss + employer_ss + eib (already correct, v_sum_employer_ss includes EIB)
...
DECLARE
  ...
  -- Penalty base amounts (for transparency)
...
BEGIN
  ...
  IF v_days_late > 0 THEN
    -- Levy Penalty base = employee_levy + employer_levy + severance_amount
    v_levy_penalty_base := COALESCE(v_sum_employee_levy, 0) + COALESCE(v_sum_employer_levy, 0) + COALESCE(v_sum_employer_severance, 0);
    ...
    -- Severance Penalty base = severance_amount only
    ...
    -- SS Fine base = employee_ss + employer_ss + eib (v_sum_employer_ss already includes EIB)
