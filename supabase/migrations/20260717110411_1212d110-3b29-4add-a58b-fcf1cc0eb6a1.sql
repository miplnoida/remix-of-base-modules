...
BEGIN
  ...
  DO UPDATE SET display_name = 'View', is_enabled = true
  ...
  -- Deterministic reaffirmation: for every role that already holds any
  -- create/edit/delete grant on bn_awards_list, ensure the view grant exists
  -- and is set to is_granted=true. Uses DO UPDATE so a previously revoked
  -- row is corrected. Other roles are NOT touched.
  FOR v_role IN
  ...
  -- Canonical Admin role: same deterministic reaffirmation.
