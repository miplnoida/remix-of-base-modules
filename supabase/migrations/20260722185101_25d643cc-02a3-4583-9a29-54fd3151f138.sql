...
DECLARE
  ...
  v_map_template_id uuid;
  v_map_template_code text;
  ...
  v_cert_kind      text;
  v_cert_hash      text;
  v_cert_is_stale  boolean;

  v_fresh_status   text;
  v_fresh_changed  text[];
...
BEGIN
  IF v_target_stage NOT IN (
    ...
    RETURN jsonb_build_object(
      'ready', false, 'target_stage', v_target_stage,
  ...
  SELECT id, active, template_id, template_code
    INTO v_map_id, v_map_active, v_map_template_id, v_map_template_code
  ...
  IF v_map_id IS NULL THEN
    v_blockers := v_blockers || jsonb_build_array(jsonb_build_object(
      ...
      'severity','critical','stage',v_target_stage));
    v_ready := false;
  ELSIF NOT COALESCE(v_map_active,false) THEN
    v_blockers := v_blockers || jsonb_build_array(jsonb_build_object(
      'code','EVENT_MAPPING_NOT_ACTIVE',
  ...
  IF v_map_id IS NOT NULL THEN
    ...
    IF v_template_id IS NULL THEN
      SELECT id INTO v_template_id FROM public.core_template
       WHERE template_code = v_map_template_code LIMIT 1;
    END IF;
    ...
    IF v_template_id IS NOT NULL THEN
      ...
      IF v_version_id IS NULL THEN
        v_blockers := v_blockers || jsonb_build_array(jsonb_build_object(
          ...
          'severity','critical','stage',v_target_stage));
        v_ready := false;
      ELSIF LOWER(COALESCE(v_version_status,'')) NOT IN ('active','approved_internal','approved_external') THEN
        v_blockers := v_blockers || jsonb_build_array(jsonb_build_object(
          'code','TEMPLATE_VERSION_NOT_ACTIVE',
  ...
  IF v_version_id IS NOT NULL THEN
    ...
    SELECT id, certification_kind, dependency_hash, is_stale
      INTO v_cert_id, v_cert_kind, v_cert_hash, v_cert_is_stale
    ...
    ELSE
      ...
      SELECT freshness_status, changed_dependency_categories
        INTO v_fresh_status, v_fresh_changed
        FROM public.comm_hub_certification_freshness
       WHERE certification_id = v_cert_id;

      IF COALESCE(v_cert_is_stale,false)
         OR (v_current_hash IS NOT NULL AND v_current_hash <> v_cert_hash)
         OR (v_fresh_status IS NOT NULL AND v_fresh_status IN ('STALE','SUPERSEDED')) THEN
        v_blockers := v_blockers || jsonb_build_array(jsonb_build_object(
          'code',
            CASE WHEN v_fresh_status = 'SUPERSEDED'
  ...
  IF v_target_stage IN ('PREVIEW_APPROVAL','DRY_RUN_TEST','CONTROLLED_STUB') THEN
    IF p_preview_snapshot_id IS NULL THEN
      ...
      v_ready := false;
    ELSE
      SELECT id, governance_certification_id, certified_dependency_hash
        INTO v_snap_id, v_snap_cert, v_snap_hash
  ...
  RETURN jsonb_build_object(
    ...
    'channel',     v_channel,
    'event_template_map_id', v_map_id,
    'mapping_active', COALESCE(v_map_active,false),
    'template_id', v_template_id,
    'template_version_id', v_version_id,
    'template_version_status', v_version_status,
    'governance_record_id', v_gov_id,
    'governance_status', v_gov_status,
    'certification_id', v_cert_id,
    'certification_kind', v_cert_kind,
    'certified_dependency_hash', v_cert_hash,
    'current_dependency_hash', v_current_hash,
    'certification_freshness',
      COALESCE(v_fresh_status,
               CASE WHEN COALESCE(v_cert_is_stale,false) THEN 'STALE' ELSE 'CURRENT' END,
               'NOT_EVALUATED'),
