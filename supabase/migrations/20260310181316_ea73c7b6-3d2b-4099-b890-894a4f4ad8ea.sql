CREATE OR REPLACE FUNCTION public.calculate_c3_contributions(p_period_year integer, p_period_month integer, p_received_date date, p_employee_data jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
...
BEGIN
  ...
  LOOP
    ...
    IF v_holiday > 0 AND v_holiday_has_dates AND v_monday_count > 0 THEN
      ...
      -- For monthly employees, only count holiday days that fall within this month.
      -- If holiday dates span across month boundaries, prorate the amount.
      IF v_pay_period_code = 'M' THEN
        v_month_start := v_period_date;  -- 1st of the submission month
        v_month_end := (v_period_date + INTERVAL '1 month' - INTERVAL '1 day')::DATE;  -- last day of month
        
        v_total_holiday_days := (v_hol_end - v_hol_start) + 1;
        
        -- Check if holiday dates extend outside this month
