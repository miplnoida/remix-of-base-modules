...
-- Fix: Drop ALL overloads (single-param and two-param) to clear parameter defaults
-- Root cause: previous migration created functions with DEFAULT NULL on p_email,
-- which PostgreSQL treats as a single-param overload. DROP (TEXT,TEXT) alone doesn't
-- remove it, so CREATE OR REPLACE fails with "cannot remove parameter defaults".
...
-- ============================================================
-- Recreate: public_api_er_master_details (no DEFAULT on p_email)
-- ============================================================
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'mobile', COALESCE(TRIM(v_rec.mobile), ''),
    'fax', COALESCE(TRIM(v_rec.fax), ''),
...
-- ============================================================
-- Recreate: public_api_se_master_details (no DEFAULT on p_email)
-- ============================================================
