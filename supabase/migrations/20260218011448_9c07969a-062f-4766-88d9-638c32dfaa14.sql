...
-- Create the fixed convert_application_to_ip and validate_application_for_conversion RPCs

CREATE FUNCTION public.convert_application_to_ip(
...
BEGIN
  IF EXISTS (SELECT 1 FROM ip_master WHERE application_reference_number = p_reference_number) THEN
    RETURN jsonb_build_object('success', false, 'error', 'DUPLICATE_CONVERSION',
  ...
  INSERT INTO ip_master (
    ...
    p_photo_url,
    p_approved_by, now(), CURRENT_DATE,
  ...
  IF p_dependants IS NOT NULL AND jsonb_array_length(p_dependants) > 0 THEN
    FOR v_dep IN SELECT * FROM jsonb_array_elements(p_dependants) LOOP
      BEGIN
        INSERT INTO ip_depend (ssn, depend_id, depend_ssn, firstname, surname, dob, sex, relation,
          depend_addr1, school_child, invalid, status, tran_code, userid, date_modified)
        VALUES (
          v_new_id::text, 'DEP-' || LPAD(v_dep_seq::text, 3, '0'),
          ...
          CASE WHEN (v_dep->>'dateOfBirth') IS NOT NULL AND (v_dep->>'dateOfBirth') != ''
          ...
          'N', 'P', 'ADD',
          LEFT(COALESCE(p_approved_by::text, 'CONV'), 5), now()
        );
        v_dep_seq   := v_dep_seq + 1;
        v_dep_count := v_dep_count + 1;
      EXCEPTION WHEN OTHERS THEN
        RAISE WARNING 'convert_application_to_ip: dep % failed ref %: % (SQLSTATE: %)',
  ...
  RETURN jsonb_build_object('success', true, 'ip_master_id', v_new_id,
    'application_id', v_application_id, 'dependants_converted', v_dep_count,
    'message', format('Application %s successfully converted to IP record', p_reference_number));

EXCEPTION WHEN OTHERS THEN
  RAISE WARNING 'convert_application_to_ip FATAL ref %: % (SQLSTATE: %)', p_reference_number, SQLERRM, SQLSTATE;
...
-- ──────────────────────────────────────────────────────────────────
-- validate_application_for_conversion: centralized pre-flight check
-- ──────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  ELSIF LENGTH(TRIM(p_first_name)) > 25 THEN
    v_warnings := v_warnings || jsonb_build_object('field','firstName','type','LENGTH',
      'message','First name has ' || LENGTH(TRIM(p_first_name)) || ' chars; will be truncated to 25.');
  END IF;
  ...
  ELSIF LENGTH(TRIM(p_last_name)) > 25 THEN
    v_warnings := v_warnings || jsonb_build_object('field','lastName','type','LENGTH',
      'message','Last name has ' || LENGTH(TRIM(p_last_name)) || ' chars; will be truncated to 25.');
  END IF;
  ...
  ELSE
    ...
    EXCEPTION WHEN OTHERS THEN
      v_errors := v_errors || jsonb_build_object('field','dateOfBirth','type','INVALID',
        'message','Date of birth is not a valid date: "' || p_date_of_birth || '".');
    END;
  END IF;
  ...
  -- Marital Status (NOT NULL varchar 20)
  IF p_marital_status IS NULL OR TRIM(p_marital_status) = '' THEN
    v_warnings := v_warnings || jsonb_build_object('field','maritalStatus','type','MISSING',
      'message','Marital status missing; will default to Single.');
  END IF;

  -- Email (varchar 40 — CRITICAL: often exceeds limit!)
  IF p_email IS NOT NULL AND LENGTH(TRIM(p_email)) > 40 THEN
    v_errors := v_errors || jsonb_build_object('field','email','type','LENGTH',
      'message','Email address has ' || LENGTH(TRIM(p_email)) || ' chars but limit is 40. Conversion will fail unless shortened.');
  END IF;

  -- Contact email (varchar 40)
  IF p_contact_email IS NOT NULL AND LENGTH(TRIM(p_contact_email)) > 40 THEN
    v_errors := v_errors || jsonb_build_object('field','contactEmail','type','LENGTH',
      'message','Contact email has ' || LENGTH(TRIM(p_contact_email)) || ' chars but limit is 40.');
  END IF;

  -- Address fields (varchar 30 each — CRITICAL: often exceeds limit!)
  IF p_address_line1 IS NOT NULL AND LENGTH(TRIM(p_address_line1)) > 30 THEN
    v_warnings := v_warnings || jsonb_build_object('field','addressLine1','type','LENGTH',
      'message','Residential address line 1 has ' || LENGTH(TRIM(p_address_line1)) || ' chars; will be truncated to 30.');
  END IF;
  IF p_address_line2 IS NOT NULL AND LENGTH(TRIM(p_address_line2)) > 30 THEN
    v_warnings := v_warnings || jsonb_build_object('field','addressLine2','type','LENGTH',
      'message','Residential address line 2 has ' || LENGTH(TRIM(p_address_line2)) || ' chars; will be truncated to 30.');
  END IF;
  IF p_mailing_addr1 IS NOT NULL AND LENGTH(TRIM(p_mailing_addr1)) > 30 THEN
    v_warnings := v_warnings || jsonb_build_object('field','mailingAddr1','type','LENGTH',
      'message','Mailing address line 1 has ' || LENGTH(TRIM(p_mailing_addr1)) || ' chars; will be truncated to 30.');
  END IF;
  IF p_mailing_addr2 IS NOT NULL AND LENGTH(TRIM(p_mailing_addr2)) > 30 THEN
    v_warnings := v_warnings || jsonb_build_object('field','mailingAddr2','type','LENGTH',
      'message','Mailing address line 2 has ' || LENGTH(TRIM(p_mailing_addr2)) || ' chars; will be truncated to 30.');
  END IF;

  -- Contact fields
  IF p_contact_name IS NOT NULL AND LENGTH(TRIM(p_contact_name)) > 35 THEN
    v_warnings := v_warnings || jsonb_build_object('field','contactName','type','LENGTH',
      'message','Contact name has ' || LENGTH(TRIM(p_contact_name)) || ' chars; will be truncated to 35.');
  END IF;
  IF p_contact_addr1 IS NOT NULL AND LENGTH(TRIM(p_contact_addr1)) > 30 THEN
    v_warnings := v_warnings || jsonb_build_object('field','contactAddress','type','LENGTH',
      'message','Contact address line 1 has ' || LENGTH(TRIM(p_contact_addr1)) || ' chars; will be truncated to 30.');
  END IF;
  IF p_contact_addr2 IS NOT NULL AND LENGTH(TRIM(p_contact_addr2)) > 30 THEN
    v_warnings := v_warnings || jsonb_build_object('field','contactAddress1','type','LENGTH',
      'message','Contact address line 2 has ' || LENGTH(TRIM(p_contact_addr2)) || ' chars; will be truncated to 30.');
  END IF;

  -- Relation name fields (varchar 35)
  IF p_father_name IS NOT NULL AND LENGTH(TRIM(p_father_name)) > 35 THEN
    v_warnings := v_warnings || jsonb_build_object('field','fatherName','type','LENGTH',
      'message','Father name has ' || LENGTH(TRIM(p_father_name)) || ' chars; will be truncated to 35.');
  END IF;
  IF p_mother_name IS NOT NULL AND LENGTH(TRIM(p_mother_name)) > 35 THEN
    v_warnings := v_warnings || jsonb_build_object('field','motherName','type','LENGTH',
      'message','Mother name has ' || LENGTH(TRIM(p_mother_name)) || ' chars; will be truncated to 35.');
  END IF;
  IF p_spouse_name IS NOT NULL AND LENGTH(TRIM(p_spouse_name)) > 35 THEN
    v_warnings := v_warnings || jsonb_build_object('field','spouseName','type','LENGTH',
      'message','Spouse name has ' || LENGTH(TRIM(p_spouse_name)) || ' chars; will be truncated to 35.');
  END IF;
  IF p_spouse_addr1 IS NOT NULL AND LENGTH(TRIM(p_spouse_addr1)) > 30 THEN
    v_warnings := v_warnings || jsonb_build_object('field','spouseAddress','type','LENGTH',
      'message','Spouse address has ' || LENGTH(TRIM(p_spouse_addr1)) || ' chars; will be truncated to 30.');
  END IF;
  IF p_spouse_ssn IS NOT NULL AND LENGTH(REGEXP_REPLACE(TRIM(p_spouse_ssn), '[^0-9]', '', 'g')) > 6 THEN
    v_warnings := v_warnings || jsonb_build_object('field','spouseSSN','type','LENGTH',
      'message','Spouse SSN exceeds 6 digit limit; will be truncated.');
  END IF;
  IF p_beneficiary_name IS NOT NULL AND LENGTH(TRIM(p_beneficiary_name)) > 35 THEN
    v_warnings := v_warnings || jsonb_build_object('field','beneficiaryName','type','LENGTH',
      'message','Beneficiary name has ' || LENGTH(TRIM(p_beneficiary_name)) || ' chars; will be truncated to 35.');
  END IF;
  IF p_ben_addr1 IS NOT NULL AND LENGTH(TRIM(p_ben_addr1)) > 30 THEN
    v_warnings := v_warnings || jsonb_build_object('field','beneficiaryAddress','type','LENGTH',
      'message','Beneficiary address has ' || LENGTH(TRIM(p_ben_addr1)) || ' chars; will be truncated to 30.');
  END IF;
  IF p_witness_name IS NOT NULL AND LENGTH(TRIM(p_witness_name)) > 35 THEN
    v_warnings := v_warnings || jsonb_build_object('field','witnessName','type','LENGTH',
      'message','Witness name has ' || LENGTH(TRIM(p_witness_name)) || ' chars; will be truncated to 35.');
  END IF;
  ...
  IF p_remarks IS NOT NULL AND LENGTH(TRIM(p_remarks)) > 250 THEN
    v_warnings := v_warnings || jsonb_build_object('field','remarks','type','LENGTH',
      'message','Remarks have ' || LENGTH(TRIM(p_remarks)) || ' chars; will be truncated to 250.');
  END IF;

  -- Occupation (varchar 4)
  IF p_occupation IS NOT NULL AND LENGTH(TRIM(p_occupation)) > 4 THEN
    v_warnings := v_warnings || jsonb_build_object('field','occupationCode','type','LENGTH',
      'message','Occupation code "' || p_occupation || '" exceeds 4 chars; will be truncated.');
  END IF;
  ...
  IF p_dependants IS NOT NULL AND jsonb_array_length(p_dependants) > 0 THEN
    FOR v_dep IN SELECT * FROM jsonb_array_elements(p_dependants) LOOP
      ...
      IF (v_dep->>'firstName') IS NULL OR TRIM(v_dep->>'firstName') = '' THEN
        v_errors := v_errors || jsonb_build_object('field','dependants[' || v_dep_idx || '].firstName',
          'type','MISSING','message','Dependant #' || v_dep_idx || ': First name is required.');
      END IF;
      IF (v_dep->>'lastName') IS NULL OR TRIM(v_dep->>'lastName') = '' THEN
        v_errors := v_errors || jsonb_build_object('field','dependants[' || v_dep_idx || '].lastName',
          'type','MISSING','message','Dependant #' || v_dep_idx || ': Last name is required.');
      END IF;
      IF (v_dep->>'dateOfBirth') IS NOT NULL AND TRIM(v_dep->>'dateOfBirth') != '' THEN
        BEGIN
          PERFORM (v_dep->>'dateOfBirth')::DATE;
        EXCEPTION WHEN OTHERS THEN
          v_errors := v_errors || jsonb_build_object('field','dependants[' || v_dep_idx || '].dateOfBirth',
            'type','INVALID','message','Dependant #' || v_dep_idx || ': Invalid date of birth "' || (v_dep->>'dateOfBirth') || '".');
        END;
      END IF;
      IF (v_dep->>'relationship') IS NULL OR TRIM(v_dep->>'relationship') = '' THEN
        v_warnings := v_warnings || jsonb_build_object('field','dependants[' || v_dep_idx || '].relationship',
          'type','MISSING','message','Dependant #' || v_dep_idx || ': Relationship code is missing.');
      END IF;
    END LOOP;
  END IF;
...
EXCEPTION WHEN OTHERS THEN
  RETURN jsonb_build_object(
    ...
    'errors', jsonb_build_array(jsonb_build_object('field','system','type','SYSTEM_ERROR',
      'message','Validation service error: ' || SQLERRM)),
    'warnings', '[]'::JSONB, 'error_count', 1, 'warning_count', 0
  );
END;
$$;

GRANT EXECUTE ON FUNCTION public.convert_application_to_ip TO authenticated, anon;
GRANT EXECUTE ON FUNCTION public.validate_application_for_conversion TO authenticated, anon;
