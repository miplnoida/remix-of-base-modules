...
-- ============================================================
-- RPC 1: Insert C3 Reported Record
-- ============================================================
...
BEGIN
  ...
  v_period_ts := p_period::TIMESTAMP;
  ...
  INSERT INTO public.cn_c3_reported (
    ...
    p_payer_id, p_payer_type, v_period_ts::TEXT, v_seq, 'DEL',
...
-- ============================================================
-- RPC 2: Insert IP Wages with Parent Validation
-- ============================================================
...
BEGIN
  ...
  -- Parent validation: check cn_c3_reported exists with DEL status
  ...
  INSERT INTO public.ip_wages (
    ...
    p_ssn, p_payer_id, p_payer_type, p_sequence_no, v_period_ts::TEXT, 'DEL', v_c3_id,
...
-- ============================================================
-- RPC 3: Verify C3 with recalculation and nil_return handling
-- ============================================================
...
BEGIN
  ...
  IF v_wages_count = 0 THEN
    RETURN jsonb_build_object(
      'status', 'error',
      'message', 'No wage records found for the given payer_id, payer_type, period, and sequence_no. Cannot verify C3 without wages.'
    );
  END IF;

  -- Recalculate totals from ip_wages
  SELECT
    COALESCE(SUM(
      COALESCE(wages_paid1, 0) + COALESCE(wages_paid2, 0) + COALESCE(wages_paid3, 0) +
      COALESCE(wages_paid4, 0) + COALESCE(wages_paid5, 0) + COALESCE(wages_paid6, 0)
    ), 0),
    COALESCE(SUM(
      COALESCE(er_ss_amt, 0) + COALESCE(er_ei_amt, 0) + COALESCE(ip_ss_amt, 0)
    ), 0),
    COALESCE(SUM(
      COALESCE(er_levy_amt, 0) + COALESCE(ip_levy_amt, 0)
    ), 0),
    COALESCE(SUM(
      COALESCE(ip_pe_amt, 0)
    ), 0)
  ...
  -- Update cn_c3_reported with recalculated values and verify
  UPDATE public.cn_c3_reported
  SET posting_status = 'VAC',
      date_verified = NOW()::TEXT,
      verified_by = 'API',
  ...
  -- Mark all matching ip_wages as verified
...
-- ============================================================
-- Register the 3 APIs in api_registry
-- ============================================================
INSERT INTO public.api_registry (api_name, api_version, http_method, endpoint_path, description, requires_auth, is_enabled, category, sort_order, created_by)
VALUES
  ('Insert C3 Reported', 'v1', 'POST', '/api/v1/c3-reported', 'Insert a new C3 reported record with auto-generated sequence number. Forces posting_status=DEL.', true, true, 'C3 Ingestion', 200, 'SYSTEM'),
  ('Insert C3 Wages', 'v1', 'POST', '/api/v1/c3-wages', 'Insert a wage record linked to an existing C3 reported record. Validates parent C3 exists with DEL status.', true, true, 'C3 Ingestion', 201, 'SYSTEM'),
  ('Verify C3', 'v1', 'POST', '/api/v1/c3-verify', 'Verify a C3 by recalculating totals from wage records. Handles nil-return and non-nil-return flows.', true, true, 'C3 Ingestion', 202, 'SYSTEM');
