...
-- Add base_currency and currency_conversion_rate to c3_payment_methods
ALTER TABLE public.c3_payment_methods
  ADD COLUMN IF NOT EXISTS base_currency TEXT NOT NULL DEFAULT 'XCD';

-- Add base_currency and currency_conversion_rate to cn_payment (split detail rows)
ALTER TABLE public.cn_payment
  ADD COLUMN IF NOT EXISTS base_currency TEXT NOT NULL DEFAULT 'XCD',
  ADD COLUMN IF NOT EXISTS currency_conversion_rate NUMERIC NOT NULL DEFAULT 1,
  ADD COLUMN IF NOT EXISTS currency_code TEXT NOT NULL DEFAULT 'XCD';

-- Recreate RPC to resolve base currency server-side from tb_currencies
...
BEGIN
  ...
  -- Recalculate base_amount server-side for the split
  v_method_currency := COALESCE(v_meth_arr[1]->>'currency_code', v_base_currency);
  SELECT exchange_rate INTO v_method_rate
  FROM public.tb_currencies
  WHERE currency_code = v_method_currency AND is_active = true LIMIT 1;
  ...
  LOOP
    ...
    IF v_alloc > 0 THEN
      ...
      -- Resolve rate for this method at split time
