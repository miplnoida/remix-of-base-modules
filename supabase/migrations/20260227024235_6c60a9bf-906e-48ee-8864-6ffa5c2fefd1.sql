...
DECLARE
  ...
  -- Monthly ceiling override
  v_monthly_total_for_levy NUMERIC;
  v_use_monthly_override BOOLEAN;
  v_levy_slab_code_to_use VARCHAR;
  -- Bi-weekly grouping
  v_biweekly_amounts NUMERIC[];
...
BEGIN
  ...
  LOOP
    ...
    -- ===== EMPLOYEE LEVY (pay-period-aware slab-based) =====
    v_employee_levy := 0;
    v_bonus_levy := 0;
    
    IF NOT v_is_age_exempt_levy THEN
      -- Calculate monthly total for ceiling check (weeks + holiday, excluding bonus)
      v_monthly_total_for_levy := v_week1 + v_week2 + v_week3 + v_week4 + v_week5 + v_holiday;
      
      -- Determine if monthly override applies
      ...
      IF v_use_monthly_override THEN
        -- MONTHLY OVERRIDE: calculate levy on total monthly amount using M slabs
        v_week_levy := 0;
        ...
        END IF;
      ELSE
        -- STANDARD: calculate using pay-period-specific slabs per payment
        CASE v_pay_period_code
          WHEN 'W' THEN
            -- Weekly: apply W slab to each individual week + holiday separately
          ...
          WHEN 'E2W' THEN
            -- Bi-Weekly: group weeks into bi-weekly amounts, apply E2W slab per group
            -- Pair weeks: (w1+w2), (w3+w4), w5 standalone if present
            -- Holiday is a separate payment
            v_biweekly_amounts := ARRAY[]::NUMERIC[];
            IF (v_week1 + v_week2) > 0 THEN
              v_biweekly_amounts := v_biweekly_amounts || (v_week1 + v_week2);
            END IF;
            IF (v_week3 + v_week4) > 0 THEN
              v_biweekly_amounts := v_biweekly_amounts || (v_week3 + v_week4);
            END IF;
            IF v_week5 > 0 THEN
              v_biweekly_amounts := v_biweekly_amounts || v_week5;
            END IF;
            IF v_holiday > 0 THEN
              v_biweekly_amounts := v_biweekly_amounts || v_holiday;
            END IF;
            
            FOR v_i IN 1..array_length(v_biweekly_amounts, 1) LOOP
              v_week_amount := v_biweekly_amounts[v_i];
              v_week_levy := 0;
              IF v_week_amount > 0 THEN
                SELECT * INTO v_levy_slab
                FROM public.tb_levy_slab_details
          ...
          WHEN '2M' THEN
            -- 2-Monthly: apply 2M slab to total (weeks + holiday combined)
          ...
          WHEN 'M' THEN
            -- Monthly: apply M slab to total (weeks + holiday combined)
            v_week_amount := v_monthly_total_for_levy;
            IF v_week_amount > 0 THEN
              SELECT * INTO v_levy_slab
              FROM public.tb_levy_slab_details
      ...
      -- Bonus levy: use bonus policy (independent of pay period slabs)
