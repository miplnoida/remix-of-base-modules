...
DECLARE
  ...
  v_user_id uuid;
BEGIN
  -- Resolve user UUID from user_code or treat as UUID directly
  BEGIN
  ...
  -- Create workflow instance (started_by is uuid, started_by_name is text)
  ...
  -- workflow_logs uses user_id (uuid), user_name (text), comments (text) — not performed_by/details
