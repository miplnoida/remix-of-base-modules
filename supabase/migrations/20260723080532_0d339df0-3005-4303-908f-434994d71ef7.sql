-- Safe helper stubs used by the deterministic go-live simulator when no richer builders are defined.
CREATE OR REPLACE FUNCTION public.build_comm_hub_recipient_context(p_recipient_policy_id uuid)
RETURNS jsonb LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path TO 'public' AS $$
DECLARE v jsonb; r record;
BEGIN
  SELECT * INTO r FROM public.communication_hub_recipient_policy WHERE id = p_recipient_policy_id LIMIT 1;
  IF NOT FOUND THEN
    RETURN jsonb_build_object('mode','UNKNOWN','address',NULL,'display_name',NULL);
  END IF;
  v := jsonb_build_object(
    'policy_id', r.id,
    'mode', r.active_mode,
    'address', r.single_configured_address,
    'display_name', r.single_configured_display_name,
    'display_name_confirmed', coalesce(r.single_configured_display_name_confirmed,false),
    'policy_version', r.policy_version
  );
  RETURN v;
END; $$;

CREATE OR REPLACE FUNCTION public.build_comm_hub_request_context(p_source text DEFAULT 'deterministic-runner')
RETURNS jsonb LANGUAGE sql STABLE SECURITY DEFINER SET search_path TO 'public' AS $$
  SELECT jsonb_build_object(
    'source', coalesce(p_source,'deterministic-runner'),
    'requested_at', to_char(now() AT TIME ZONE 'UTC','YYYY-MM-DD"T"HH24:MI:SS"Z"'),
    'trace_id', gen_random_uuid()::text
  );
$$;

CREATE OR REPLACE FUNCTION public.build_comm_hub_system_context()
RETURNS jsonb LANGUAGE sql STABLE SECURITY DEFINER SET search_path TO 'public' AS $$
  SELECT jsonb_build_object(
...
GRANT EXECUTE ON FUNCTION public.build_comm_hub_recipient_context(uuid) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.build_comm_hub_request_context(text)      TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.build_comm_hub_system_context()           TO authenticated, service_role;

-- Update the runner so certification drift only flags when a certified hash exists on record.
...
BEGIN
  ...
  IF v_tv_id IS NOT NULL THEN
    ...
    ELSE
      ...
      v_cert_id := v_cert.id;
      SELECT dependency_hash INTO v_current_dep_hash FROM public.build_comm_hub_certification_dependency_hash(v_tv_id);
      -- Only flag drift when a hash was recorded on the certification and it disagrees with the current one.
      IF v_cert.dependency_hash IS NOT NULL
         AND v_current_dep_hash IS NOT NULL
         AND v_cert.dependency_hash <> v_current_dep_hash THEN
        v_blockers := v_blockers || jsonb_build_array(jsonb_build_object(
          'stage','template_certification',
  ...
  RETURN jsonb_build_object(
    ...
    'template_certification', jsonb_build_object(
       ...
       'hash_match', CASE WHEN v_cert.dependency_hash IS NULL OR v_current_dep_hash IS NULL THEN NULL
                          ELSE v_cert.dependency_hash = v_current_dep_hash END
    ),
