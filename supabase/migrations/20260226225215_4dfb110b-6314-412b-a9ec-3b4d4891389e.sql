...
-- Fix levy penalty base: REMOVE severance from levy penalty base
-- Levy Penalty base = employee_levy + employer_levy ONLY (no severance)
-- Severance Penalty base = severance_amount only (unchanged)
-- SS Fine base = employee_ss + employer_ss_total (includes EIB) (unchanged)
...
BEGIN
  ...
  IF v_days_late > 0 THEN
    -- FIX: Levy Penalty base = employee_levy + employer_levy ONLY (severance excluded)
    -- Severance has its own separate penalty, so it must NOT be double-counted in levy penalty
    ...
    -- SS Fine base = employee_ss + employer_ss_total (employer_ss_total already includes EIB)
