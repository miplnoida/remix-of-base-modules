...
-- Create the convert_application_to_employer RPC
-- This function atomically creates an er_master record + owners + locations + notes
...
CREATE OR REPLACE FUNCTION public.convert_application_to_employer(
  ...
  p_males_employed DOUBLE PRECISION DEFAULT NULL,
  p_females_employed DOUBLE PRECISION DEFAULT NULL,
...
BEGIN
  ...
  IF p_application_reference IS NOT NULL AND p_application_reference <> '' THEN
    -- Check if an er_master already exists with this registry_num (used to store app ref)
    IF EXISTS (
