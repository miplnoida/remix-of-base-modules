...
-- Fix: Remove non-existent columns (payer_type, payer_id, inserted_by, insert_date) from cn_payment INSERT
-- Keep currency_code which exists. Keep sequence_no in c3_payment_components which already exists.
...
BEGIN
  ...
  -- Step 1: Create payment header (payer_type and payer_id stored HERE)
  ...
  -- Step 2: Save C3 entry structure (includes sequence_no)
  ...
  LOOP
    ...
    IF v_alloc > 0 THEN
      ...
      -- FIXED: Removed payer_type, payer_id, inserted_by, insert_date (don't exist in cn_payment)
      -- Payer info is already stored in cn_payment_header
