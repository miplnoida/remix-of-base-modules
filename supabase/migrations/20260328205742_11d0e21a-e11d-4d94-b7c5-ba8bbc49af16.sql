...
-- Add suggested_days column to candidates table for days-based planning
ALTER TABLE public.ia_auto_plan_candidates 
  ADD COLUMN IF NOT EXISTS suggested_days numeric,
  ADD COLUMN IF NOT EXISTS suggested_reviewer_id uuid,
  ADD COLUMN IF NOT EXISTS suggested_quarter text,
  ADD COLUMN IF NOT EXISTS suggested_month text,
  ADD COLUMN IF NOT EXISTS coverage_category text,
  ADD COLUMN IF NOT EXISTS suggested_scope text,
  ADD COLUMN IF NOT EXISTS suggested_objectives text;

-- ============================================================
-- Enhanced: ia_convert_candidates_to_engagements
-- Now populates ALL key engagement fields from candidate + function data
-- ============================================================
...
BEGIN
  ...
  FOR v_candidate IN
    SELECT c.*, df.function_name, df.risk_rating AS func_risk_rating, 
           ...
           d.name AS dept_name, d.head_name AS dept_head
  ...
  LOOP
    ...
    -- Derive quarter and month from suggested_start_date
    ...
    -- Estimate days based on risk level (if not explicitly set)
    ...
    -- Auto-generate scope from function description
    ...
    -- Build inclusion rationale from reason codes
    v_rationale := '';
    ...
    -- Coverage category from function or risk-based default
...
-- ============================================================
-- Enhanced: ia_generate_auto_plan_candidates 
-- Now also populates suggested_days, quarter, month, coverage, scope, objectives
-- ============================================================
...
BEGIN
  ...
  FOR v_func IN
    SELECT df.id AS function_id, df.department_id, df.function_name, df.description AS func_desc,
           df.risk_rating AS func_risk, d.name AS dept_name, d.head_name AS dept_head
  ...
  LOOP
    ...
    -- Determine risk label and estimated days
    v_risk_label := CASE
    ...
    -- Stagger scheduling: high-risk in Q1/Q2, medium in Q2/Q3, low in Q3/Q4
    ...
    -- Calculate slot dates based on quarter staggering
