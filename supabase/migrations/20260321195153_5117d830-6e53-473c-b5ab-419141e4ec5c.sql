...
-- Add cancel/reprint fields to cn_invoices (mirroring cn_receipt pattern)
ALTER TABLE public.cn_invoices
  ADD COLUMN IF NOT EXISTS cancel_date TIMESTAMPTZ,
  ADD COLUMN IF NOT EXISTS cancel_reason TEXT,
  ADD COLUMN IF NOT EXISTS cancel_user TEXT,
  ADD COLUMN IF NOT EXISTS reprint_times INTEGER NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS updated_by TEXT,
  ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ;

-- Update create_invoice_with_lines RPC to set initial status = 'O'
...
BEGIN
  ...
  -- Insert invoice header with status = 'O' (Original)
