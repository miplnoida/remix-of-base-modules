...
DECLARE
  ...
  -- Holiday target slot for without_dates merge
...
BEGIN
  ...
  -- ===== BONUS POLICY: exception first, then default =====
  ...
  LOOP
    ...
    -- ===== DETERMINE HOLIDAY TARGET SLOT from policy levy_distribution =====
    -- For without_dates + merge: holiday goes to the configured target slot
    -- For with_dates: already distributed above, v_holiday=0
    v_hp_target_slot := 0; -- 0 means no specific slot (fallback: add to monthly total)
    IF v_hp_eligible AND v_hp_levy_include AND v_hp_levy_method = 'merge' AND v_hp_levy_distribution IS NOT NULL THEN
      IF v_pay_period_code = 'W' THEN
        -- Weekly: find the selected week (w1-w4), default to last (w4)
        ...
        ELSE v_hp_target_slot := 4; -- default: last week
        END IF;
      ELSIF v_pay_period_code = 'E2W' THEN
        -- Bi-weekly: b1→slot 1 (wk1+wk2), b2→slot 3 (wk3+wk4)
        ...
        ELSE v_hp_target_slot := 3; -- default: last bi-weekly slot
        END IF;
      ELSIF v_pay_period_code = '2M' THEN
        -- Semi-monthly: s1→slot 1, s2→slot 3
        ...
        ELSE v_hp_target_slot := 3; -- default: last semi-monthly slot
        END IF;
      ELSE
        -- Monthly: m1 → slot 0 (add to monthly total, handled naturally)
        v_hp_target_slot := 0;
      END IF;
    END IF;
    ...
    IF NOT v_is_age_exempt_levy THEN
      ...
      -- Add holiday to monthly total if merge (or if not included in levy at all - it's part of wages)
      ...
      IF v_bonus_eligible AND v_bonus_include_in_levy AND v_bp_calculation_method = 'merge' THEN
        -- ===== MERGE BONUS =====
        -- Build merged amounts: holiday goes to target slot (not separate element 6)
        v_merged_amounts := ARRAY[v_week1, v_week2, v_week3, v_week4, v_week5, 0::NUMERIC];
        
        -- Place holiday into the correct slot based on policy distribution
        IF v_hp_eligible AND v_hp_levy_include AND v_hp_levy_method = 'merge' THEN
          ...
          ELSE
            -- Monthly or no target: add to element 6 as fallback
            v_merged_amounts[6] := v_holiday;
          END IF;
        ELSIF v_hp_eligible AND v_hp_levy_include AND v_hp_levy_method = 'separate' THEN
          -- Separate: holiday excluded from merged amounts (calculated separately later)
          v_merged_amounts[6] := 0;
        ELSE
          -- Holiday not included in levy policy OR no policy: treat as regular wages
          -- Add to target slot if available, else keep as separate element
          IF v_hp_target_slot > 0 AND v_hp_target_slot <= 5 THEN
            v_merged_amounts[v_hp_target_slot] := v_merged_amounts[v_hp_target_slot] + v_holiday;
          ELSE
            v_merged_amounts[6] := v_holiday;
        ...
        -- Distribute bonus per bonus policy distribution
        IF v_pay_period_code = 'W' THEN
      ...
      ELSIF v_bonus_eligible AND v_bonus_include_in_levy AND v_bp_calculation_method = 'separate' THEN
        ...
        ELSE
          v_slab_code := v_pay_period_code;
          -- Build amounts array with holiday merged into target slot
          ...
          IF NOT (v_hp_eligible AND v_hp_levy_include AND v_hp_levy_method = 'separate') THEN
            -- Holiday is merge or not in levy: add to target slot
            IF v_hp_target_slot > 0 AND v_hp_target_slot <= 5 THEN
      ...
      ELSE
        ...
        ELSE
          ...
          v_amounts_array := ARRAY[v_week1, v_week2, v_week3, v_week4, v_week5];
          IF NOT (v_hp_eligible AND v_hp_levy_include AND v_hp_levy_method = 'separate') THEN
            -- Holiday is merge or not in levy: add to target slot from policy
            IF v_hp_target_slot > 0 AND v_hp_target_slot <= 5 THEN
              v_amounts_array[v_hp_target_slot] := v_amounts_array[v_hp_target_slot] + v_holiday;
            ELSE
              v_amounts_array := v_amounts_array || v_holiday;
