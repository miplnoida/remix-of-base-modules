...
-- 1. Additive columns on communication_message (only truly missing ones).
...
CREATE INDEX IF NOT EXISTS communication_message_dispatch_idx
  ON public.communication_message (status, next_attempt_at)
  WHERE status IN ('queued','sending');

CREATE INDEX IF NOT EXISTS communication_message_origin_idx
  ON public.communication_message (origin);

-- 2. Roll-up helper.
CREATE OR REPLACE FUNCTION public.recompute_communication_request_status(p_request_id uuid)
RETURNS text
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  v_new           text;
BEGIN
  SELECT status INTO v_current FROM public.communication_request WHERE id = p_request_id;
  ...
  SELECT
    ...
    COUNT(*) FILTER (WHERE status IN ('failed','bounced','suppressed','cancelled')),
    COUNT(*) FILTER (WHERE status IN ('sent','delivered','failed','bounced','suppressed','cancelled')),
    COUNT(*) FILTER (WHERE status IN ('queued','sending'))
  INTO v_total, v_sent, v_failed, v_terminal, v_in_flight
  ...
  IF v_terminal = v_total AND v_failed = v_total THEN
    v_new := 'failed';
  ELSIF v_terminal = v_total AND v_sent = v_total THEN
    v_new := 'completed';
  ELSIF v_terminal = v_total THEN
    v_new := 'partial';
  ELSIF v_in_flight > 0 AND v_current <> 'pending' THEN
    v_new := 'dispatching';
  ELSE
    v_new := v_current;
  END IF;
...
REVOKE ALL ON FUNCTION public.recompute_communication_request_status(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.recompute_communication_request_status(uuid) TO authenticated, service_role;

-- 3. Secure enqueue RPC.
...
BEGIN
  ...
  END IF;
  -- Validate all channels are in the CHECK-constrained vocabulary.
  ...
  INSERT INTO public.communication_request(
    ...
    v_country_code, v_language_code, to_jsonb(v_channels_text), v_priority,
...
GRANT EXECUTE ON FUNCTION public.send_communication_v1(jsonb) TO authenticated, service_role;
