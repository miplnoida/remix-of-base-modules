...
BEGIN
  SELECT id INTO v_template_id
   ...
   WHERE code = 'COMM_HUB_ADMIN_TEST_NOTICE_EMAIL'
   LIMIT 1;
  ...
  IF v_template_id IS NULL THEN
    INSERT INTO public.core_template(
      ...
      country_code, template_type, template_category,
      status, scope, is_active,
      owner_scope, business_category,
      created_by, updated_by
    ) VALUES (
      'COMM_HUB_ADMIN_TEST_NOTICE_EMAIL',
      'Communication Hub Admin Test Notice',
      'Internal admin test notice used by Control Center to exercise the façade → resolver → dispatcher path.',
      'COMM_HUB', 'Communication Hub',
      'KN', 'EMAIL', 'ADMIN_TEST',
      'ACTIVE', 'COUNTRY', true,
      'MODULE', 'COMM_HUB',
      'phase-1c-b9-a1', 'phase-1c-b9-a1'
    )
  ...
  IF v_version_id IS NULL THEN
    INSERT INTO public.core_template_version(
      template_id, version_no, status,
      subject, body_html, body_text,
      change_summary, published_at, published_by,
      created_by, updated_by, body_metadata
    ) VALUES (
      v_template_id, 1, 'PUBLISHED',
      'Communication Hub Admin Test Notice',
      $HTML$<div>
<p>Hello {{recipient_name}},</p>
<p>This is an internal Communication Hub admin test notice.</p>
<p>It validates the façade, resolver, template version, dispatcher, dry-run delivery attempt, audit trail, and operational visibility.</p>
<p><strong>Request:</strong> {{request_no}}<br/>
<strong>Generated at:</strong> {{generated_at}}</p>
</div>$HTML$,
      $TXT$Hello {{recipient_name}},

This is an internal Communication Hub admin test notice.

It validates the façade, resolver, template version, dispatcher, dry-run delivery attempt, audit trail, and operational visibility.

Request: {{request_no}}
Generated at: {{generated_at}}$TXT$,
      'Initial seed (Phase 1C-B9-A.1).',
      now(), 'phase-1c-b9-a1',
      'phase-1c-b9-a1', 'phase-1c-b9-a1',
      jsonb_build_object(
        'required_tokens', jsonb_build_array('recipient_name','request_no','generated_at'),
        'module_code','COMM_HUB',
        'event_code','ADMIN_TEST_NOTICE',
        'channel','EMAIL'
      )
    )
  ...
  UPDATE public.core_template
   ...
   WHERE id = v_template_id
     AND (active_version_id IS DISTINCT FROM v_version_id);
END $$;

-- 2) Patch send_communication_v1 (backward-compatible additions).
...
BEGIN
  ...
  UPDATE public.communication_request SET status = 'queued' WHERE id = v_request_id;
