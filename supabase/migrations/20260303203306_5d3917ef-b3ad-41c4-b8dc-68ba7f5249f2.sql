...
-- Fix: DATE::INTEGER cast error in holiday distribution overlap calculation.
-- PostgreSQL date subtraction (date - date) returns integer days directly.
-- This was causing the entire distribution to fail silently, leaving holiday
-- pay as a standalone amount not merged into weekly slots for levy calculation.
      ...
      IF v_hp_distribution_enabled THEN
        ...
        FOR v_i IN 1..v_monday_count LOOP
          ...
          IF v_hol_start <= v_week_end AND v_hol_end >= v_week_start THEN
            -- FIX: use date subtraction (date - date = integer) instead of DATE::INTEGER cast
