...
-- Patch runner: entity_version must be text
DO $$
BEGIN
  -- no-op guard
  NULL;
...
-- Only alter the INSERT expression via CREATE OR REPLACE (replay full function)
...
BEGIN
  ...
  IF v_reg IS NULL THEN v_blockers := v_blockers || jsonb_build_array(jsonb_build_object('stage','module_event_registration','code','event_not_registered')); END IF;
  ...
  IF v_live IS NULL THEN v_blockers := v_blockers || jsonb_build_array(jsonb_build_object('stage','event_live_control','code','event_live_control_missing')); END IF;
  ...
  IF v_map IS NULL THEN v_blockers := v_blockers || jsonb_build_array(jsonb_build_object('stage','event_template_map','code','event_template_map_missing_or_inactive')); END IF;
  IF v_map IS NOT NULL THEN
    ...
    IF v_tv IS NULL THEN v_blockers := v_blockers || jsonb_build_array(jsonb_build_object('stage','template_version','code','template_active_version_missing'));
    ELSE
      v_tv_id := v_tv.id;
  ...
  IF v_schema IS NULL THEN v_blockers := v_blockers || jsonb_build_array(jsonb_build_object('stage','event_payload_schema','code','event_payload_schema_missing'));
  ...
  IF v_fixture IS NULL THEN v_blockers := v_blockers || jsonb_build_array(jsonb_build_object('stage','test_scenario','code','governed_test_scenario_missing'));
  ELSE
  ...
  IF v_recipient IS NULL THEN v_blockers := v_blockers || jsonb_build_array(jsonb_build_object('stage','recipient_policy','code','recipient_policy_missing'));
  ELSE
  ...
  IF v_map IS NOT NULL AND v_map.sender_profile_id IS NOT NULL THEN
    ...
    IF v_sender IS NULL THEN v_blockers := v_blockers || jsonb_build_array(jsonb_build_object('stage','sender_profile','code','sender_profile_missing'));
    ELSE
  ...
  IF v_tv_id IS NOT NULL THEN
    ...
    IF v_gov IS NULL THEN v_blockers := v_blockers || jsonb_build_array(jsonb_build_object('stage','governance','code','template_version_governance_missing')); END IF;
    SELECT * INTO v_cert FROM comm_hub_certification WHERE entity_type='TEMPLATE_VERSION' AND entity_id=v_tv_id ORDER BY certified_at DESC LIMIT 1;
    IF v_cert IS NULL THEN v_blockers := v_blockers || jsonb_build_array(jsonb_build_object('stage','governance','code','template_version_certification_missing'));
    ELSIF v_cert.result NOT IN ('PASS','WARN') THEN v_blockers := v_blockers || jsonb_build_array(jsonb_build_object('stage','governance','code','template_version_certification_not_pass','details',v_cert.result));
    ELSIF v_cert.is_stale=true THEN v_blockers := v_blockers || jsonb_build_array(jsonb_build_object('stage','governance','code','template_version_certification_stale','details',v_cert.stale_reason)); END IF;
  END IF;
  ...
  v_gov_record_id := _comm_hub_ensure_event_governance_record(v_map.id, v_hash);
  ...
  INSERT INTO comm_hub_certification(
    ...
    v_gov_record_id, 'EVENT_TEMPLATE_MAPPING', coalesce(v_map.id, gen_random_uuid()),
