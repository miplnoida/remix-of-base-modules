...
BEGIN
  ...
  IF v_ver.id IS NULL THEN
    v_blockers := array_append(v_blockers, 'template_version_missing'::text);
  END IF;
  ...
  LOOP
    ...
    IF coalesce(v_token_val,'') = '' THEN
      v_missing := array_append(v_missing, v_token_key);
    END IF;
  END LOOP;
  ...
  FOREACH v_dummy_hit IN ARRAY ARRAY['dry-run','dry run','dummy','lorem ipsum','placeholder','test only','sample only'] LOOP
    IF position(lower(v_dummy_hit) IN lower(v_subject || ' ' || v_body_text || ' ' || v_body_html)) > 0 THEN
      v_warnings := array_append(v_warnings, ('dummy_wording:' || v_dummy_hit)::text);
    END IF;
  END LOOP;
  ...
  IF v_review IS NULL THEN
    v_blockers := array_append(v_blockers, 'review_policy_missing'::text);
  END IF;
  IF v_review IS NOT NULL AND (v_review->>'approval_status') NOT IN ('approved_internal','approved_external') THEN
    v_blockers := array_append(v_blockers, 'template_not_approved'::text);
  END IF;
  IF v_review IS NOT NULL AND (v_review->>'approved_template_version_id') IS NOT NULL
    ...
    v_warnings := array_append(v_warnings, 'active_version_differs_from_approved'::text);
  END IF;
  IF array_length(v_unresolved,1) > 0 THEN
    v_blockers := array_append(v_blockers, 'unresolved_tokens_present'::text);
  END IF;
  IF v_recipient_email = '' THEN
    v_blockers := array_append(v_blockers, 'recipient_email_missing'::text);
  END IF;
