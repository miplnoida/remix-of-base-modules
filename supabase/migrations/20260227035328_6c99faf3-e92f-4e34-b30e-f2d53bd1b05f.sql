...
DECLARE
  ...
  -- Due month for calendar-month penalty calculation
  v_due_year INTEGER;
  v_due_month INTEGER;
...
BEGIN
  ...
  WHERE is_active = true
    AND v_period_date >= date_from::date
    AND (date_to IS NULL OR date_to = '' OR v_period_date <= date_to::date)
  ...
  -- 3. Calculate due date (last day of month following C3 period)
  ...
  -- 4. Calculate days late (kept for display) and calendar months late (for penalties)
  v_days_late := GREATEST(0, p_received_date - v_due_date);
  
  -- Calendar-month-based lateness calculation:
  -- Due month = month after C3 period (e.g., C3=Aug 2025 → due month=Sep 2025)
  -- If submitted in due month → 0 months late
  -- If submitted in next month → 1 month late (10% initial)
  -- Each additional calendar month → +1 (1% additional per month)
  ...
  v_months_late_calendar := (v_received_year * 12 + v_received_month) - (v_due_year * 12 + v_due_month);
  IF v_months_late_calendar < 0 THEN
    v_months_late_calendar := 0;
  END IF;
  
  -- Keep legacy day-based values for backward compat display
  ...
  -- 5. Process each employee (LEVY LOGIC UNCHANGED)
  ...
  LOOP
    ...
    -- ===== EMPLOYEE LEVY (per-payment slab-based - UNCHANGED) =====
    v_employee_levy := 0;
    v_bonus_levy := 0;
    ...
    IF NOT v_is_age_exempt_levy THEN
      ...
      IF v_use_monthly_override THEN
        ...
        WHERE slab_id = v_config.levy_slab_id
          AND pay_period = 'M'
          AND is_active = true
          AND v_monthly_total_for_levy > over_amt
      ...
      ELSE
        IF v_pay_period_code = 'M' THEN
          v_week_amount := v_monthly_total_for_levy;
          IF v_week_amount > 0 THEN
            ...
            LIMIT 1;
            IF v_levy_slab IS NOT NULL THEN
              v_employee_levy := ROUND(v_levy_slab.base_amt + ((v_week_amount - v_levy_slab.over_amt + 0.01) * v_levy_slab.tax_rate), 2);
            END IF;
          END IF;
        ELSE
  ...
  -- ===== PENALTY CALCULATIONS (FIXED: calendar-month-based) =====
  -- Calendar months late: compare submission month to due month
  -- Due month = month after C3 period. If submitted in due month → 0.
  -- First late month → initial rate. Each additional month → subsequent rate.
  
  IF v_months_late_calendar > 0 THEN
    -- Levy penalty: base = employee levy + employer levy
    ...
    v_levy_penalty := ROUND(
      ...
      v_levy_penalty_base * v_config.levy_penalty_subsequent_rate * GREATEST(v_months_late_calendar - 1, 0),
      2
    );
    
    -- Severance penalty: base = employer severance only
    ...
    v_severance_penalty := ROUND(
      ...
      v_severance_penalty_base * v_config.severance_penalty_subsequent_rate * GREATEST(v_months_late_calendar - 1, 0),
      2
    );
    
    -- SS fine: base = employee SS + employer SS total (includes EIB)
    -- SS uses its own rates but same calendar-month timeline
    v_ss_fine_base := COALESCE(v_sum_employee_ss, 0) + COALESCE(v_sum_employer_ss, 0);
    v_ss_fine := ROUND(
      v_ss_fine_base * v_config.ss_fine_initial_rate +
      v_ss_fine_base * COALESCE(v_config.ss_fine_subsequent_rate, v_config.ss_fine_initial_rate) * GREATEST(v_months_late_calendar - 1, 0),
      2
    );
