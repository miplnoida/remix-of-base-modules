...
BEGIN
  ...
  ELSE
    ...
    ELSE
      ...
      BEGIN
        v_recomputed_hash := (public.comm_hub_normalize_recipient_set(
      ...
      END;
      IF v_recomputed_hash IS NOT NULL
         AND COALESCE(v_snap.recipient_set_hash,'') <> ''
      ...
      END IF;
      v_hash_match := (v_recomputed_hash IS NOT NULL
    ...
    ELSE
      ...
      ELSE
        FOR v_r IN SELECT jsonb_array_elements(v_snap.to_recipients  || v_snap.cc_recipients || v_snap.bcc_recipients) LOOP
          IF jsonb_typeof(v_r) = 'string' THEN
            v_addr := lower(btrim(v_r #>> '{}'));
          ELSE
  ...
  v_evidence := jsonb_build_object(
    ...
    'recipient_snapshot_recomputed_hash', v_recomputed_hash,
    'recipient_snapshot_hash_match', v_hash_match,
    'frozen_recipient_snapshot_available', v_recipients_valid AND COALESCE(v_snap.recipient_set_hash,'')<>'',
    'recipient_snapshot_valid', v_recipients_valid,
