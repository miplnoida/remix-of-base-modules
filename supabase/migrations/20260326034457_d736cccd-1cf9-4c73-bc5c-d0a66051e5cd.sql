...
BEGIN
  ...
  -- Resolve function_ids: either the single function or all functions in the department
  ...
  -- 1. Risk score (ia_risk_assessments has function_id, not department_id)
  SELECT COALESCE(AVG(
    CASE ra.risk_level
  ...
  WHERE ra.is_active = true
    AND ra.function_id = ANY(COALESCE(v_function_ids, ARRAY[]::uuid[]));
