...
-- ============================================================
-- Update convert_application_atomic to persist verification_type
-- from the p_documents JSONB array into ip_application_documents
-- ============================================================
...
BEGIN
  ...
  -- Insert documents (with verification_type)
  IF p_documents IS NOT NULL AND jsonb_array_length(p_documents) > 0 THEN
    FOR v_doc IN SELECT * FROM jsonb_array_elements(p_documents) LOOP
      ...
      INSERT INTO ip_application_documents (
        ssn, application_reference_number, document_name, document_type, file_name, file_path, url, signed_url, mime_type, file_size, uploaded_at, source_document_id, verification_type, metadata, created_by
      )
      VALUES (
        v_final_ssn,
