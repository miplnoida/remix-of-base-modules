...
BEGIN
  ...
  IF v_existing_instance_id IS NOT NULL THEN
    INSERT INTO system_audit_trail (
      entity_type, entity_id, action, new_value, performed_by_user_id, performed_by_user_code, source_context
    ) VALUES (
      'workflow_instance', v_existing_instance_id::TEXT, 'workflow_initiation_skipped_duplicate',
      jsonb_build_object('reason', 'instance_already_exists', 'source_record_id', p_unique_uuid)::TEXT,
  ...
  IF v_trigger IS NULL THEN
    INSERT INTO system_audit_trail (
      ...
      jsonb_build_object('module_id', v_module_id, 'action', 'submit')::TEXT,
      p_user_id, p_user_code, p_source_context
    );
    RETURN NULL;
  END IF;
  ...
  -- Fixed: use user_id/user_name instead of performed_by/performed_by_name
