...
-- Add new columns to ip_master for application conversion tracking
ALTER TABLE public.ip_master 
  ADD COLUMN IF NOT EXISTS application_reference_number VARCHAR(50),
  ADD COLUMN IF NOT EXISTS application_remarks TEXT;

-- Index for duplicate prevention and search
CREATE UNIQUE INDEX IF NOT EXISTS idx_ip_master_app_ref_number 
  ON public.ip_master (application_reference_number) 
  WHERE application_reference_number IS NOT NULL;

-- RPC function: Convert approved online application to ip_master record
...
BEGIN
  ...
  INSERT INTO ip_master (
    ...
    -- Address
    resident_address_1, resident_address_2, postal_district, mailing_address,
    ...
    -- Address
    p_address_line1, p_address_line2, p_postal_district, p_mailing_addr1,
