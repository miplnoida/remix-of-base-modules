...
BEGIN
  ...
  -- EPIC PROD-2D: evaluator raise now fail-closes live sends (below).
  IF v_is_live THEN
    DECLARE
      v_gate_status   jsonb;
    ...
    BEGIN
      ...
      EXCEPTION WHEN OTHERS THEN
        -- EPIC PROD-2D: live-only fail-closed on evaluator error.
        v_gate_status := NULL;
        BEGIN
          INSERT INTO public.communication_hub_control_audit
            (setting_key, old_value, new_value, reason, changed_by, source)
          VALUES
            ('send_communication_v1.runtime_gate.eval_error',
             jsonb_build_object('module_code',v_module_code,'event_code',v_event_code,'is_live',v_is_live),
             to_jsonb(SQLERRM),
             'Runtime gate evaluator raised; live send blocked for safety (PROD-2D)',
             v_requested_by,
        ...
        BEGIN
          IF v_trace_id IS NOT NULL THEN
            PERFORM public.append_comm_hub_trace_step(v_trace_id, jsonb_build_object(
              ...
              'status','blocked',
              'blocker_codes', jsonb_build_array(jsonb_build_object(
                'code','runtime_gate_evaluator_error',
                'severity','critical',
                'message','Runtime gate evaluator failed; live send blocked for safety.'
              )),
              'plain_summary','Runtime gate evaluator raised; live send blocked (PROD-2D fail-closed).',
              'payload', jsonb_build_object(
            ...
            PERFORM public.complete_comm_hub_trace(v_trace_id,'blocked',
              jsonb_build_object('blocked_stage','DB_RUNTIME_GATE_CHECKED','reason','runtime_gate_evaluator_error'));
          END IF;
        EXCEPTION WHEN OTHERS THEN NULL;
        END;
        RAISE EXCEPTION 'send_communication_v1: runtime gate evaluator failed; live send blocked for safety.'
          USING ERRCODE = '42501';
      END;
