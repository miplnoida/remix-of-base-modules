...
DECLARE
  ...
  v_current_hash text; v_manifest jsonb;
...
BEGIN
  ...
  ELSIF NOT COALESCE(v_map_active,false) THEN
    v_blockers := v_blockers || jsonb_build_array(jsonb_build_object('code','EVENT_MAPPING_NOT_ACTIVE',
      'message','Event mapping exists but is not active','severity','critical','stage',v_target_stage));
    v_ready := false;
  END IF;
  ...
  IF v_version_id IS NOT NULL THEN
    ...
    SELECT id,certification_kind,dependency_hash,is_stale INTO v_cert_id,v_cert_kind,v_cert_hash,v_cert_is_stale
      FROM public.comm_hub_certification
     WHERE entity_type='TEMPLATE_VERSION'::comm_hub_governance_entity_type AND entity_id=v_version_id
       AND result='PASS' AND superseded_by IS NULL
    ...
    IF v_cert_id IS NULL THEN
      v_blockers := v_blockers || jsonb_build_array(jsonb_build_object('code','TEMPLATE_VERSION_NOT_CERTIFIED',
        'message','No current PASS certification exists for this template version','severity','critical','stage',v_target_stage));
      v_ready := false;
    ELSE
      BEGIN
        v_manifest := public.build_comm_hub_dependency_manifest('TEMPLATE_VERSION',v_version_id,v_version_id);
      EXCEPTION WHEN others THEN v_manifest := NULL; END;
  ...
  RETURN jsonb_build_object(
    ...
    'certification_freshness',
      COALESCE(v_fresh_status, CASE WHEN COALESCE(v_cert_is_stale,false) THEN 'STALE' ELSE 'CURRENT' END, 'NOT_EVALUATED'),
    ...
    'source', 'check_comm_hub_runtime_governance', 'evaluator_version', '4b3.slice1',
    'evaluated_at', now()
  );
END; $$;
